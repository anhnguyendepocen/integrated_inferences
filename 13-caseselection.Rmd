...
In this example we sought to observe $M$ for four units, one in each possible $X$, $Y$ combination. This gives rise to 16 distinct possible data observations.

```{r, echo = FALSE}
df <- possible_data
colnames(df) <- c("Event", 1:16)
kable(df, caption = "Possible data from observing M for one case in each X,Y cell")
```
