...
```{r, echo=FALSE}
if(do_diagnosis){
diagnosis_X1Y0_n1 <- diagnose_strategy(
									reference_model = reference_model, 
                  analysis_model  = model,
                  given = given, 
                  queries = "Y[X=1]>Y[X=0]",
									possible_data_args = list(vars = "M",
																						within = TRUE, 
																						N = 1,
																						condition = "X==1 & Y==0" ))	
 write_rds(diagnosis_X1Y0_n1, "saved/diagnosis_n1_X1Y0.rds")
	
} 
diagnosis_X1Y0_n1 <- read_rds("saved/diagnosis_n1_X1Y0.rds")
```
...
```{r, echo=FALSE}
if(do_diagnosis){
diagnosis_X0Y1_n1 <- diagnose_strategy(
									reference_model = reference_model, 
                  analysis_model  = model,
                  given = given, 
                  queries = "Y[X=1]>Y[X=0]",
									possible_data_args = list(vars = "M",
																						...
																						N = 1,
																						condition = "X==0 & Y==1" ))	
 write_rds(diagnosis_X0Y1_n1, "saved/diagnosis_n1_X0Y1.rds")
	
}
diagnosis_X0Y1_n1 <- read_rds("saved/diagnosis_n1_X0Y1.rds")
```
...
```{r, echo=FALSE}
if(do_diagnosis){
diagnosis_X0Y0_n1 <- diagnose_strategy(
									reference_model = reference_model, 
                  analysis_model  = model,
                  given = given, 
                  queries = "Y[X=1]>Y[X=0]",
									possible_data_args = list(vars = "M",
																						...
																						N = 1,
																						condition = "X==0 & Y==0" ))	
 write_rds(diagnosis_X0Y0_n1, "saved/diagnosis_n1_X0Y0.rds")
}

diagnosis_X0Y0_n1 <- read_rds("saved/diagnosis_n1_X0Y0.rds")
```


```{r, echo=FALSE}
diagnoses_n1 <- do.call(rbind, 
                     lapply(list(diagnosis_X1Y1, 
                                 diagnosis_X1Y0_n1, 
                                 diagnosis_X0Y1_n1,
                                 diagnosis_X0Y0_n1), 
                            function(x) x$diagnosis ))
kable(diagnoses_n1, digits = 3)
```


<!-- output not ready for 2 steps strategies)-->

```{r, echo=FALSE}
if(do_diagnosis){
diagnosis_X1Y1_n2 <- diagnose_strategy(
    									reference_model = reference_model, 
                      analysis_model  = model,
                      given = given, 
                      queries = "Y[X=1]>Y[X=0]",
    									possible_data_args = list(vars = "M",
    																						...
    																						N = 2,
    																						condition = "X==1 & Y==1" ))	
 write_rds(diagnosis_X1Y1_n2, "saved/diagnosis_n2_X1Y1.rds")
	
}
diagnosis_X1Y1_n2 <- read_rds("saved/diagnosis_n2_X1Y1.rds")
```

```{r, echo=FALSE}
if(do_diagnosis){
diagnosis_X0Y1_X1Y1 <- diagnose_strategy(
    									reference_model = reference_model, 
                      analysis_model  = model,
                      given = given, 
                      queries = "Y[X=1]>Y[X=0]",
    									possible_data_args = list(vars = "M",
    																						within = TRUE, 
    																						N = 2,
    																						condition = "(X==0 & Y==1) | (X==1 & Y==1)"))
 write_rds(diagnosis_X0Y1_X1Y1, "saved/diagnosis_n2_X0Y1_X1Y1.rds")
	
} 
diagnosis_X0Y1_X1Y1 <- read_rds("saved/diagnosis_n2_X1Y0_X1Y1.rds")
```
...
```{r, echo=FALSE}
if(do_diagnosis){
diagnosis_X1Y0_X1Y1 <- diagnose_strategy(
    									reference_model = reference_model, 
                      analysis_model  = model,
                      given = given, 
                      queries = "Y[X=1]>Y[X=0]",
    									possible_data_args = list(vars = "M",
    																						within = TRUE, 
    																						N = 2,
    																						condition = "(X==1 & Y==0) | (X==1 & Y==1)" ))	
 write_rds(diagnosis_X1Y0_X1Y1, "saved/diagnosis_n2_X1Y0_X1Y1.rds")
	
}
diagnosis_X1Y0_X1Y1  <- read_rds("saved/diagnosis_n2_X1Y0_X1Y1.rds")
```

```{r, echo=FALSE}
if(do_diagnosis){
diagnosis_X0Y1_X1Y0 <- diagnose_strategy(
    									reference_model = reference_model, 
                      analysis_model  = model,
                      given = given, 
                      queries = "Y[X=1]>Y[X=0]",
    									possible_data_args = list(vars = "M",
    																						within = TRUE, 
    																						N = 2,
    																						condition = "(X==1 & Y==0) | (X==0 & Y==1)" ) )	
 write_rds(diagnosis_X0Y1_X1Y0, "saved/diagnosis_n2_X0Y1_X1Y0.rds")
	
} 
diagnosis_X0Y1_X1Y0<- read_rds("saved/diagnosis_n2_X1Y0_X1Y0.rds")
```

```{r, echo=FALSE}
if(do_diagnosis){
diagnosis_X0Y1_X0Y0 <- diagnose_strategy(
    									reference_model = reference_model, 
                      analysis_model  = model,
                      given = given, 
                      queries = "Y[X=1]>Y[X=0]",
    									possible_data_args =  list(vars = "M",
    																						within = TRUE, 
    																						N = 2,
    																						condition = "(X==0 & Y==1) | (X==0 & Y==0)"  ))	
 write_rds(diagnosis_X0Y1_X0Y0, "saved/diagnosis_n2_X0Y1_X0Y0.rds")
	
} 
diagnosis_X0Y1_X0Y0 <- read_rds("saved/diagnosis_n2_X0Y1_X0Y0.rds")
```
...
```{r, echo=FALSE}
if(do_diagnosis){
diagnosis_X1Y0_X0Y0 <- diagnose_strategy(
    									reference_model = reference_model, 
                      analysis_model  = model,
                      given = given, 
                      queries = "Y[X=1]>Y[X=0]",
    									possible_data_args = list(vars = "M",
    																						...
    																						N = 2,
    																						condition = "(X==0 & Y==1) | (X==0 & Y==0)"  ))	
 write_rds(diagnosis_X1Y0_X0Y0, "saved/diagnosis_n2_X1Y0_X0Y0.rds")
	
}
diagnosis_X1Y0_X0Y0  <- read_rds("saved/diagnosis_n2_X1Y0_X0Y0.rds")
```


```{r, echo=FALSE}
if(do_diagnosis){
diagnosis_X0Y0_n2 <- diagnose_strategy(
    									reference_model = reference_model, 
                      analysis_model  = model,
                      given = given, 
                      queries = "Y[X=1]>Y[X=0]",
    									possible_data_args = list(vars = "M",
    																						...
    																						N = 2,
    																						condition = "X==0 & Y==0" ))	
 write_rds(diagnosis_X0Y0_n2, "saved/diagnosis_n2_X0Y0.rds")
}

diagnosis_X0Y0_n2 <- read_rds("saved/diagnosis_n2_X0Y0.rds")
```


```{r, echo=FALSE}
diagnoses_n2 <- do.call(rbind, 
                     lapply(list(diagnosis_X1Y1_n2, 
                                 diagnosis_X1Y0_X1Y1, 
                                 diagnosis_X0Y1_X1Y1,
                                 diagnosis_X0Y1_X1Y0,
                                 diagnosis_X1Y0_X0Y0, 
