...
# The reference model is the original model updated with the XY data we've already seen.
 ...
 probabilities <- lapply(strategies, function(s) 
    average_data_probabilities(reference_model, s, using = "posteriors", sims = 1000)
    )
  ...
  df <- rbind(df, 
          ...
          c("Posterior mean",   round(unlist(lapply(estimates_db, function(j)   unlist(j[,4]))),2)),
...
kable(df, caption = "Each column shows a possible distribution of data that can be generated from a given strategy. We calculate the probability of each data possibility, given the data seen so far, and the posterior variance associated with each one.")
```
...
kable(data.frame(Strategy = c("On the line", "Off the line", "X=1, Y=1 cases"), Variance = expected_posterior_var))
```
