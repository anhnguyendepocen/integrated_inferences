...
To illustrate the procedure, suppose that we have initially observed $X$ and $Y$ data on 6 cases. The initial data are summarized in Table \@ref(tab:6xycases).
...
As we can see, two cases lie in each of the on-diagonal cells of an $X,Y$ table, those where $X=Y$, and one case lies in each of the off-diagonal cells (where $X \neq Y$). Suppose that we are now considering gathering process-tracing evidence for 1 of these cases. We have many different case-selection strategies we might pursue, and each of these can give rise to different possible data and thus to different possible conclusions. What should we do?
...
**Probability of the data**. We now calculate a probability of each possible data realization, given the model and the data (the $X$'s and $Y$'s) that we have already observed. In practice, we do this in `gbiqq` via simulation. Starting with the model together with our priors, we update based on the initial $X,Y$ data. We then use this posterior---representing what we know about the world having seen the $X,Y$ data---to draw a series of $\lambda$'s: share allocations of causal types. And given that we have (from an ambiguity matrix) the mapping from causal types to data realizations, we can calculate for each $lambda$ draw the probabiltiy of each data possibility. 


FLAG: Make table to go with text. Columns are: Strategy, Possible Data [with row split into M=0, M=1], Probability of Data (for each M value), Posterior Variance (for each M value), Expected Posterior Variance (one value per strategy, weighted average over M values).
...
if(do_diagnosis){
	...
	estimates_database_n1_X1Y1 <- make_estimates_database(
                  model, 
                  given, 
  ...
  diagnosis_X1Y1 <- diagnose_strategy(
	...
	write_rds(estimates_database_n1_X1Y1, "saved/estimates_database_n1_X1Y1.rds")
	write_rds(diagnosis_X1Y1, "saved/diagnosis_n1_X1Y1.rds")
}
...
diagnosis_X1Y1             <- read_rds("saved/diagnosis_n1_X1Y1.rds")
