...
case_selection_diagnose <- function(model, given,  N, sims = 4000, ...)
...
if(do_diagnosis){
  ...
  for(N in c(2, 4)) { for(m in  1:length(models)) {for(g in 1:length(givens)) {	
...
df %>%
  ...
  #  theme_pubclean()+
  facet_grid(rows = vars(Query), cols = vars(given))
```
...
long_data <-  expand_data(gen_given(4,2,2,4), model)
updated <- gbiqq(model, long_data, iter = 14000, chains = 12) %>%
...
update_check <- gbiqq(models$base_model, data_check)
query_check <- query_model(update_check, "Y[X=1] - Y[X=0]", using = "posteriors")

# Prior check

prior_data <- gen_given(12,4,4,12)  %>% expand_data(models$base_model)
update_prior <- gbiqq(models$base_model, prior_data, chains = 8, iter = 10000)
