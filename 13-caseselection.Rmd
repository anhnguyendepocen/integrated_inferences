...
#  for(N in 4) { for(m in  1:2) {for(g in 1:length(givens)) {	
  for(N in 4) { for(m in  2) {for(g in 5) {	
    filename <-  paste0("saved/M_", names(models)[m], "_", N, "_", names(givens)[g], ".rds")
    
    print(filename)
    
    write_rds(case_selection_diagnose(models[[m]], givens[[g]], N = N, fit = fit), 
              filename)
    
  }}}

  #  for(N in c(2, 4)) { for(m in  1:length(models)) {for(g in 1:length(givens)) {	
#  for(N in 4) { for(m in  3:5) {for(g in 1:length(givens)) {	
