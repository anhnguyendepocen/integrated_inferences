...
The approach we have  described to inference always involves updating beliefs given data. But to get off the ground you need to be able to state priors on all parameters. You have to start from somewhere.

This is always the case with Bayesian infernece, but for the kinds of models and questions we are focussed on  the problem of stating priors can be more fundamental than for many Bayesian applications for two reasons. 

First the  beliefs are beliefs over the distribution of individual level effects and not just the beliefs over average effects. This puts us up against the fundamental problem of causal inference (Holland cite, Dawid cite FLAG). Second, the beliefs can do a lot of work---especially in small $n$ applications. Indeed for the the process tracing described chapters 6 and 7 [FLAG ADD REFS] the inferences are little more than conditional applications of a model.      
