...
We ask `gbiqq` now to query $\lambda$'s posterior distribution to generate posterior distributions for each of these quantities. We can define our queries quite simply in terms of the effect if interest and then "subset" for the relevant conditioning. We present the code and results of these operations below and in Figure \@ref(mixedhist).
...
prob_b <- query_distribution(
                   model = updated, 
                   using = "posteriors",
                   query = "D[I=1] > D[I=0]") #For what proportion of cases does inequality have a positive effect on democratization?

PC <- query_distribution(
                   model = updated, 
                   using = "posteriors",
                   query = "D[I=1] > D[I=0]",
                   subset = "D==1 & I==1") #For what proportion of I=D=1 cases did inequality cause democratization?
...
PC2 <- query_distribution(
                   ...
                   using = "posteriors",
                   query = "D[I=1] > D[I=0]",
                   subset = "D==1 & I==1 & M==1") #For what proportion  of I=M=D=1 cases did inequality cause democratization?

PC3 <- query_distribution(
                   model = updated, 
                   using = "posteriors",
                   query = "D[I=1] > D[I=0]",
                   subset = "D==1 & I==1 & M==0") #And what about for the I=D=1 cases where there was no mobilizaiton?

PC4 <- query_distribution(
                   model = updated, 
                   using = "posteriors",
                   query = "D[I=1] > D[I=0]",
                   subset = "D==1 & I==1 & M==1 & P==1") 

PC5 <- query_distribution(
                   model = updated, 
                   using = "posteriors",
                   query = "D[I=1] > D[I=0]",
                   subset = "D==1 & I==1 & M==1 & P==0") #And what about for the I=D=1 cases where there was no mobilizaiton?
...
hist(PC4,    main = "D=1 caused by I=1 | M = 1, P=1", xlim = c(0,1))
hist(PC5,    main = "D=1 caused by I=1 | M = 1, P=0", xlim = c(0,1))
...
PC4neg <- query_distribution(
                   ...
                   using = "posteriors",
                   query = "D[I=0] > D[I=1]",
                   subset = "D==0 & I==1 & M==1 & P==1")

PC5neg <- query_distribution(
                   model = updated, 
                   using = "posteriors",
                   query = "D[I=0] > D[I=1]",
                   subset = "D==0 & I==1 & M==0 & P==0")

```

```{r, mixedhist2, echo = FALSE}
par(mfrow = c(3,2))
hist(prob_a, main = "I=1 causes D=0", xlim = c(0,1))
hist(PCneg,     main = "D=0 caused by I=1", xlim = c(0,1))
hist(PC2neg,    main = "D=0 caused by I=1 | M = 1", xlim = c(0,1))
hist(PC3neg,    main = "D=0 caused by I=1 | M = 0", xlim = c(0,1))
hist(PC4neg,    main = "D=0 caused by I=1 | M = 1, P=1", xlim = c(0,1))
hist(PC5neg,    main = "D=0 caused by I=1 | M = 1, P=0", xlim = c(0,1))
