...
Let $n$ denote the number of nodes. Label the nodes $V_1, \dots V_n$ subject to the requirement that each nodes' parents precede it in the ordering. Let $pa_j$ denote the set of values of the parents of node $j$ and let $V_j(pa_j, \theta_t)$ denote the value of node $j$ given the values of its parents and the causal type $\theta_j$.
...
* Query $Q:E((1(Y(X=1)=1) \& (Y(X=0)=0)) - (1(Y(X=1)=0) \& 1(Y(X=0)=1))$ asks for the average treatment effect, represented here as the expected difference between positive and negative effects.
...
A last note on conditional queries. Say we are interested in a query of the form
