...
**Type reducing theorization.** There is a second way in which we might imagine a model being implied by another model that does not involve a change in nodes. Let $\Theta(M)$ denote the set of causal types in $M$. Then we can say that  $M_0$ implies $M_1$ if $\Theta(M_0)\subseteq \Theta(M_1)$. Informally this means that any relation admitted by theory $M_0$ is representable in model $M_1$, though the converse may not be true. We can think of a theory of $M_1$ as a *restriction* of ranges of $\Theta(M_1)$. 

Put differently, if two variables are independent--- or conditionally independent given a third variable---in one model, then this same relation of independence (or conditional independence) must be captured in any theory of that model. A theory can have *additional* conditional independencies not present in the higher-level model.  But we may not theorize *away*s (conditional) independencies insisted on by our higher-level claim.



Common restictions of this form include monotonicity restrictions and exclusion restrictions.  Both of these are invoked, for example, when researchers resort to theory to argue that conditions hold for an instrument to recover the average treatment effect for compliers: that is, that the instrument does not cause the outcome directly and that it has no negative effect on a treatment.
...
FLAG: Alan -- see use of specific here
...
We think we can. Using the notion of hierarchies of models we imagine  beginning with theory $M_1$ which together with data $D$ implies claim $M_2$. We then posit theory $M_0$ of $M_1$---and so $M_0$ implies $M_1$, but $M_0$, coupled with $D$, produces a claim , $M_2'$, that is different---and hopefully better---than $M_2'$. Our gain should be some summary of how much better $M_2'$ is than $M_2$.

Here is an illustration using a theory that allows use of the "front door criterion." 

Imagine we have data on three variables, $X$, $Y$, and $M$. We begin, however, with a model $M_1$: $C \rightarrow X \rightarrow Y \leftarrow C$, that includes nodes for two of the three variables, $X$ and $Y$, but not $M$, as well as a node $C$ (a confound) for which we do not have data. 

We see a strong correlation between $X$ and $Y$ and infer $M_2$: that $X$ is a likely cause $Y$. Our inference under $M_2$ is however uncertain because we are aware that the correlation may be due to the confound $C$.

Say now we posit  theory $M_0$: $A \rightarrow X \rightarrow M \rightarrow Y \leftarrow A$. This theory now lets us make better use of data $M$. If we observe for instance that $X$ and $M$ are uncorrelated, then we infer with confidence that in fact $X$ did not cause $Y$, despite the correlation. 

Thus, in return for specifying a theory for $M_1$, we have been able to make better use of data and form a more confident conclusion. In this case the theory does not alter *priors* over our query. Our prior over the effect of $X$ on $Y$ may be identical under $M_1$ and $M_0$, but our conclusions, given data, differ, because the theory lets us make use of the data in a way that we could not before.
