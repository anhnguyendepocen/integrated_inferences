...
In Chapter \@ref(illustratemodels), we described a set of theories and represented them as causal models. But so far we haven't been very explicit about what we mean by a theory or how theory maps onto a causal-model framework. 
...
To fix ideas: a simple claim might be that "*A* caused *B* in case $j$". This claim is itself a model, albeit a very simple one. The theory that supports this model might be of any of the following forms: 

* "*A* always causes *B*"
* "*A* always causes *B* whenever *C* (and *C* holds in case *j*)", or  
* "*A* invariably causes *B* and  invariably *B* causes *C*". 

These three statements all have in common that they are arguments that could be provided to support the simple claim; in each case, if you believe the theory, you believe the implication.
...
Let us say that a causal model, $\mathcal M^\prime$, is a *theory of* model $\mathcal M$ if $\mathcal M$ is implied by $\mathcal M^\prime$. $\mathcal M^\prime$ is a theory *because* it has implications. (A statement without implications we might be a conclusion, an inference, or a claim.)  A theory, $\mathcal M^\prime$, might itself sit atop---be supported by---another theory, $\mathcal M^{\prime\prime}$, that implies $\mathcal M^\prime$. To help fix the idea of a theory as "supporting" or "underlying" the model(s) it theorizes, we refer to the theory $\mathcal M^\prime$ as a *lower*-level model relative to $\mathcal M$ and refer to $\mathcal M$ as a *higher*-level model relative to its theorization, $\mathcal M^\prime$.^[We note that our definition of theory differs somewhat from that given in @pearl2009causality (p207): there a theory is a (functional) causal model and a restriction over $\times_j \mathcal{R}(U_j)$, that is, over the collection of contexts envisionable. Our definition also considers probabilistic models as theories, allowing statements such as "the average effect of $X$ on $Y$ is 0.5."] 
...
Both structural causal models (DAGs) and probabilistic causal models (DAGs with beliefs about distributions)---possibly in combination with data---imply other models. We discuss each in turn.

### Implications of structural causal models

A structural causal model can imply multiple other simpler structural causal models. Similarly, a given structural model can be implied by multiple more-detailed models. 

We imagine two forms of lower-level model, those that involve "type-splintering" and those that involve "type-reduction."

**Theorization by type-splintering.** Theorization often involves a refinement of causal types, implemented through the addition of nodes. 
Take the very simple model, $\mathcal M$, represented in Figure \@ref(fig:Highlow)(a). The model simply states that $X$ has (or *can* have) a causal effect on $Y$. 

What theories might justify $\mathcal M$? This question can be rephrased as, "What models imply model $\mathcal M$?" The figure points to two possibilities. Both models $\mathcal M^\prime$ (panel (b)) and  $\mathcal M^{\prime\prime}$ (panel (c)) imply model $\mathcal M$. They can be thought of as  *theories*, or lower-level models, of $\mathcal M$. 

Model $\mathcal M^\prime$ differs by the addition of a node, $K$, in the causal chain between $X$ and $Y$. We can say that  $\mathcal M^\prime$ is a *theory* of $\mathcal M$ for two reasons. First it provides a *justification*---if you believe  $\mathcal M^\prime$ you should believe $\mathcal M$: if $X$ affects $Y$ through $K$, then $X$ affects $Y$. But as well as a justification $\mathcal M^\prime$ also provides an *explanation* of $\mathcal M$. Suppose we already *know* that $X$ affects $Y$ but want to know *why*. If we ask, "Why does $X$ affect $Y$?", $\mathcal M^\prime$ provides an answer: $X$ affects $Y$ *because* $X$ affects $K$, and $K$ affects $Y$.

Model $\mathcal M^{\prime\prime}$ differs by the addition of a node, $C$, that moderates the effect of  $X$ on  $Y$. $\mathcal M^{\prime\prime}$ justifies $\mathcal M$ in the sense that, if you believe  $\mathcal M^{\prime\prime}$,  you should believe  $\mathcal M$. It provides an explanation of a kind also: if you believe model  $\mathcal M^{\prime\prime}$, you can now entertain the possibility that the relation between $X$ and $Y$ in a given setting is what it is because of $C$. Model $\mathcal M^{\prime\prime}$ tells us that, had $C$ been different, the causal relation between $X$ and $Y$ might have been also.  
...
Theories are “lower-level” causal models that explain or provide an account of a “higher-level”, simpler model. Two common forms of theorization involve articulating  *mediating* and *moderating* relationships:
  ...
  * *Mediation*: A mediator, $K$, can be introduced between $X$ and $Y$, thus splintering $\theta^Y$ into $\theta^K$ and $\theta^{Y_\text{lower}}$. The mediation theory thus explains the $X \rightarrow Y$ relationship.
  * *Moderation*: A component of $\theta^Y$ can be extracted and specified as a substantive variable, $C$. $C$ is now a substantively conceptualized moderator of the $X \rightarrow Y$ relationship. The moderation theory thus provides a fuller explanation of why $X$ has different effects on $Y$ in different contexts.

::::
<br>


A key feature of the moves to either $\mathcal M'$ or $\mathcal M''$ is that they require a redefinition of $\theta^Y$. That is, in changing the substantive endogenous nodes, those changes in turn imply a change in the interpretation of the exogenous nodal-type nodes pointing into existing endogenous nodes (such as $Y$ in this example). We can think of part of $\theta^Y$ as being splintered off and captured by some new element on the graph.


To see how type-splitting operates, consider first the move from $\mathcal M$ and $\mathcal M'$ in  Figure \@ref(fig:Highlow)(a). Importantly, in moving from the higher- to the lower-level model, we have effectively *split* the nodal-type term $\theta^Y$ into two parts: $\theta^{Y_\text{lower}}$ and $\theta^K$. Intuitively, in the higher-level model, $\mathcal M$, $Y$ is a function of $X$ and $\theta^Y$, the latter representing all things other than $X$ than can affect $Y$---or, equivalently, all of the (unspecified) sources of variation in $X$'s effect on $Y$. When we insert $K$ into the model, however, $X$ now does not directly affect $Y$ but only does so via $K$. Further, we model $X$ as acting on $K$ in a manner conditioned by $\theta^K$, which represents all of the (unspecified) factors determining $X$'s effect on $K$. The key thing to notice here is that $\theta^K$ now represents *a portion of the variance that $\theta^Y$ represented in the higher-level graph*: some of the variation in $X$'s effect on $Y$ now arises from variation in $X$'s effect on $K$, which is captured by $\theta^K$. So, for instance, $X$ might have no effect on $Y$ because $\theta^K$ takes on the value $\theta^K_{00}$, so that $X$ has no effect on $K$. Put differently, any effect of $X$ on $Y$ must arise from an effect of $X$ on $K$; so $\theta^K$'s value must be either $\theta^K_{01}$ or $\theta^K_{10}$ for $X$ to affect $Y$.^[As we emphasize further below, it is in fact only the random, unknown component of the $X\rightarrow K$ link that makes the addition of $K$ potentially informative as a matter of research design: if $K$ were a deterministic function of $X$ only, then knowledge of $X$ would provide full knowledge of $K$, and nothing could be learned from observing $K$.] 

What $\theta^K$ represents, then, is that part of the original $\theta^Y$ that arose from some force other than $X$ operating at the *first* step of the causal chain from $X$ to $Y$. So now $Y$'s nodal type in the lower-level graph is relabeled as $\theta^{Y_\text{lower}}$ because it is not quite the same entity as $\theta^Y$ in the higher-level graph. In the original graph, $\theta^Y$ represented *all* sources of variation in $X$'s effect on $Y$. In the lower-level model, with $K$ as mediator, $\theta^{Y_\text{lower}}$ represents only the variation in $K$'s effect on $Y$. Put differently, $\theta^Y$ has been expunged of any factors shaping the first stage of the causal process, which now reside in $\theta^K$. 

<!-- We highlight this change in $\theta^Y$'s meaning by referring in the second model to $\theta^{Y_\text{lower}}$.  -->
...
Consider next the somewhat different move from $\mathcal M$ (panel (a)) to $\mathcal M^{\prime\prime}$ (panel (c)). Here the lower-level model *specifies* one of the conditions that comprised $\theta^Y$ in the higher-level model. In specifying a moderator, $C$, we have extracted $C$ from $\theta^Y$, leaving $\theta^{Y_\text{lower}}$ to represent all factors *other than $C$* that condition $Y$'s response to its parents. More precisely, $\theta^{Y_\text{lower}}$ now represents the set of nodal types defining how $Y$ responds jointly to $X$ and $C$. Again, the relabeling as $\theta^{Y_\text{lower}}$ reflects this change in the term's meaning. Whereas in Model $\mathcal M^{\prime}$ we have extracted $\theta^K$ from $\theta^Y$, in Model $\mathcal M^{\prime\prime}$ it is $C$ itself that we have extracted from $\theta^Y$, substantively specifying what had been just a random disturbance.


<!-- We return to the quantitative implications of this redefinition of endogeneous nodes below.  -->

<!-- In model $\mathcal M$, $\theta^Y$ captures all the features of units that result in them responding in one or another way to $X$. However in model $\mathcal M'$, $\theta^{Y_{lower}}$  only needs to capture the way that $Y$ responds to $K$ --- that part of the effect of $X$ on $Y$ that operates via $X$'s impact on $K$ can be partitioned off. in model $\mathcal M''$, $\theta^{Y_{lower}}$  only needs to capture the way that $Y$ responds to $X$  goven $K$, uncertainty about the value of $K$ can be partialed out.  -->


```{r Highlow, echo = FALSE, fig.width = 7, fig.height = 5,  fig.align="center", out.width='80%', fig.cap = "Here we represent the simple claim that one variable causes another, and two theories --- lower-level models --- that could explain this claim. Both the model in panel (b) and the model in panel (c) involve theorization via type-splitting."}
...
A probabilistic model can also be implied by another probabilistic model, possibly coupled with data. 

**Theoretical implications of probabilistic models.** Consider first implications without data. Causal Model (b) in Figure \@ref(fig:Highlow), coupled with a distribution over  $\theta^K$ and  $\theta^Y_{lower}$, produces a probabilistic causal model, $\mathcal M_{higher}$. This model in turn implies a probabilistic model, $\mathcal M_{higher}$, formed from Causal Model (a) in Figure \@ref(fig:Highlow), and a *particular* distribution over $\theta^Y_{higher}$, that is, the distribution that preserves the causal relations implied by $\mathcal M_{lower}$. Thus, for instance, the probability that $X$ has a positive effect on $Y$ in $\mathcal M_{higher}$ is $\theta^{Y_{higher}}_{01}$; the probability that  $X$ has a positive effect on $Y$ in $\mathcal M_{lower}$ is  $\theta^{M_{lower}}_{01}\theta^{Y_{lower}}_{01}  + \theta^{M_{lower}}_{10}\theta^{Y_{lower}}_{10}$.  Consistency then requires that $\theta^{M_{lower}}_{01}\theta^{Y_{lower}}_{01}  + \theta^{M_{lower}}_{10}\theta^{Y_{lower}}_{10} = \theta^{Y_{higher}}_{01}$ and so the value of  $\theta^{Y_{higher}}_{01}$ is *implied* by $\theta^{M_{lower}}_{01},\theta^{Y_{lower}}_{01}, \theta^{M_{lower}}_{10},\theta^{Y_{lower}}_{10}$, but not vice versa.

**Deducing models from theory and data.** A probabilisitc causal model coupled with data  implies  another model via Bayes rule.  For this reason we can fruitfully think of an initial model as being a *theory of* an updated model, coupled with data. Thought of in this way we have clarity over what is meant when we turn to theory to support a claim, but also what is meant when we seek to justify a theory. We might imagine a scholar arguing: "$\mathcal M_1$: $X$ caused $Y$ in country $j$." When pushed for a justification for the claim they provide the lower level model: "$\mathcal M_0:$ the average effect of $X$ on $Y$ in countries with feature $C=1$ is 0.95, making it likely that $X$ caused $Y$ in this case." Here $\mathcal M_1$ is implied by $\mathcal M_0$ coupled with data $C=1$. If pushed further as to why that theory is itself credible they might point to a lower level model consisting of structural model $X\rightarrow Y \leftarrow C$ plus flat priors coupled with data on $X,Y$ and $C$.  At each stage, as more justification is provided, the researcher formally provides lower-level models.  

Moving up, as more data is provided, more "specific" higher level models emerge, justified by lower models plus data. These models are more specific in the sense that they are implied by the higher level models, plus data,  but not vice versa.  But they are also more also  (generally) more specific in a second sense: that they make stronger statements about how causal processes operate.^[This is not universally true, a point we return to below.]  They place greater weight on a smaller set of causal claims.^[In frequentist frameworks we often think of analysis as implementing up-or-down empirical tests against data to parse between theories that should be maintained and theories that should be rejected. In a Bayesian framework we think more continuously of shifting our beliefs across causal possibilities within a multi-dimensional theoretical space.] 

FLAG:  -- see use of specific here

As the simplest illustration, we might imagine beginning with an $X\rightarrow Y$ model, $\mathcal M_1$, in which, $X$ and $Y$ are binary, and we we believe that $Y$ possibly responds to $X$. If we have "flat" priors over causal types, in the sense described in chapter \@ref(bayeschapter), then our prior uncertainty over the proposition that $X$ causes $Y$, under this model, is large; as is our uncertainty that $Y=1$ is due to $X=1$ in a given case. In other words, given our theory, we are uncertain about the proposition $\mathcal M_3$: $X$ caused $Y$. However if we then receive a lot of data, $D$, showing strong relations between $X$ and $Y$, then our updated model $\mathcal M_2$, formed from combining $D$ and $\mathcal M_1$ allows us to infer that  $X$ caused $Y$ in this case with greater certainty. 

Thus our new theory $\mathcal M_2$ is (a) formally similar to $\mathcal M_1$ (b) formed as a product of past theory plus evidence, here justified by $\mathcal M_1$ given data $D$ and (c) capable of providing sharper implications than past theory.^[As a general matter an updated theory may not provider sharper claims for all queries. That is, in practice, posterio variance over queries can increase with more data. As a simple illustration; say  I start out thinking that  the probability that an outcome is due to A, B, or C is .9, .05, .05, respectively; I find evidence that convicnes me that A is not the cause, then I shift (a) to greater certainty about whether A was the cause but (b) greater uncertainty about whether B was the cause.]
...
<!-- A lower-level model, $\mathcal M^\prime$, can also be a representation of $\mathcal M$ in which a node has been introduced that permits variation in a feature of context that is fixed and taken-for-granted in $\mathcal M$. Here, $\mathcal M^\prime$ theorizes $\mathcal M$ in the sense of embedding $\mathcal M$ within a *more general* set of beliefs about how the world works. $\mathcal M$ then becomes a special case of the theorized relations, one that holds when we condition on some data, specifying some particularity of context. -->
...
<!-- $\mathcal M^\prime$ is a *theory* of $\mathcal M$ in that it, in a sense, helps explain the dependencies of $Y$ on $X$ more fully than does $\mathcal M$. -->


<!-- Since lower-level models imply higher level models, we think of theories as implying the models they are theorizing. If we believe structural causal model $\mathcal M'$, then we also must believe the structure of model $\mathcal M$. If it is possible that $X$ can affect $K$ and possible that $K$ can affect $Y$, then it is possible that $X$ can affect $Y$. Similarly, if we believe Model $\mathcal M^{\prime\prime}$, then we must also believe Model $\mathcal M$: if it is true that $X$ can affect $Y$, possibly in ways that are moderated by $C$, then it is trivially true that $X$ can affect $Y$.  -->
...
<!-- Given model $\mathcal M_1$, what kinds of lower level models can serve as theories for $\mathcal M_1$? -->
...
<!-- 2. A higher level model may be formed by conditioning on values of nodes in a lower level model. Conversely, a higher-level functional model, $\mathcal M$, can be theorized via a lower-level $\mathcal M^\prime$ in which conditions shaping the operation of the causal effect in $\mathcal M$, unspecified in $\mathcal M$, are now specified. -->

<!-- To illustrate this approach, consider again the graphs in Figure \ref{fig:K}. Above we described how the graph in  panel (a) can be produced by aggregating $U_Y^{\text{lower}}$ and $U_K$ from panel (c). An alternative possibility is to simplify by conditioning: we derive a higher-level graph from $\mathcal M^\prime$ by fixing the value of $K$. For instance, if $Y=XK+U_Y^{\text{lower}}$ in $\mathcal M'$, then at $K=1$, we have the submodel $\mathcal M_k$ in which $Y=X+U_Y^{\text{lower}}$. Note that, in generating a submodel by conditioning on $K$, we retain the term $U_Y^{\text{lower}}$ as we have not added causal force into $Y$'s unspecified parent. -->
...
We think we can. Using the notion of hierarchies of models we imagine  beginning with theory $\mathcal M_1$ which together with data $D$ implies claim $\mathcal M_2$. We then posit theory $\mathcal M_0$ of $\mathcal M_1$---and so $\mathcal M_0$ implies $\mathcal M_1$, but $\mathcal M_0$, coupled with $D$, produces a claim , $\mathcal M_2'$, that is different---and hopefully better---than $\mathcal M_2'$. Our gain should be some summary of how much better $\mathcal M_2'$ is than $\mathcal M_2$.

Here is an illustration using a theory that allows use of the "front door criterion." 

Imagine we have data on three variables, $X$, $Y$, and $K$. We begin, however, with a model $\mathcal M_1$: $C \rightarrow X \rightarrow Y \leftarrow C$, that includes nodes for two of the three variables, $X$ and $Y$, but not $K$, as well as a node $C$ (a confound) for which we do not have data. 

We see a strong correlation between $X$ and $Y$ and infer $\mathcal M_2$: that $X$ is a likely cause $Y$. Our inference under $\mathcal M_2$ is however uncertain because we are aware that the correlation may be due to the confound $C$.

Say now we posit  theory $\mathcal M_0$: $A \rightarrow X \rightarrow K \rightarrow Y \leftarrow A$. This theory now lets us make better use of data $K$. If we observe for instance that $X$ and $K$ are uncorrelated, then we infer with confidence that in fact $X$ did not cause $Y$, despite the correlation. 

Thus, in return for specifying a theory for $\mathcal M_1$, we have been able to make better use of data and form a more confident conclusion. In this case the theory does not alter *priors* over our query. Our prior over the effect of $X$ on $Y$ may be identical under $\mathcal M_1$ and $\mathcal M_0$, but our conclusions, given data, differ, because the theory lets us make use of the data in a way that we could not before.

In other examples we might imagine invoking a theory that does not necessarily involve new data but that allows us to make different, perhaps tighter inferences. An example might be the invocation of a theory that involves a monotonicity restriction, or exclusion restriction, that allows for identification of a quantity that would not be identifiable without it.    

```{r, echo = FALSE,  eval = FALSE}

M_1 <- make_model("A -> X -> Y <- A")

M_0 <- make_model("A -> X -> M -> Y <- A")

```

Thus one reason to theorize our models --- develop more elaborate, lower-level models to support them --- is to be able to reap greater inferential leverage from the more elaborate theory. 

But are we, in fact, better off? 

We might imagine answering the question from an internal or external position and from an ex ante or ex post perspective.

In all cases we ask how much better do we do as a result of making use of a theory. 

One approach for operationalizing "better" is to assess the size of the error we make, from an inference that uses a theory, compared to an inference that does not make use of the theory. That's an ex post, objective assessment. Ex ante, we might ask what the expected error is from conclusions one draws given a theory For instance: how wrong are you likely to be if you base your best guess on your posterior mean? How wrong might be operationalized in terms of mean squared error---the sqaure of the distance between the truth and the posterior mean.^[Other loss functions could be used, including functions that take account of the costs of collecting additional data or to the risks associated with false diagnoses. For instance, in @heckerman1991toward, an objective function is generated using  expected utility gains from diagnoses generated based on new information over diagnoses based on what is believed already. In their treatment [@heckerman1991toward, Equation 6],  the expected value of new information $K$, given existing information $W$ is: $\sum{K}P(K|W)( EU(d(Q,W,K)|W, K) - EU(d(Q, W)|W, K))$ where $EU$ is expected utility and $d$ is the optimal inference (diagnosis) given available data. Note that the diagnosis can take account of $K$ when it is observed, but the expected utility depends on $K$ whether or not it is observed, as $K$ carries information about the state of interest.] 
 
A more subjective approach is to ask about the reduction in posterior variance from conclusions that make use of a theory compared to conclusions that do not. That's an ex position. We might also think about  the *expected posterior variance*: how certain do you expect you will be after you make use of this new information?
...
Interestingly if you assess expectations using the same priors as you use for for forming posteriors the expected posterior variance and expected squared error  are equivalent [@scharf1991statistical].
...
One way to capture this idea that, although you are more uncertaint you think you are better off now than you were, is to ask: how much better are our guesses having observed $K$ compared to what we would have guessed before, *given* what we know having observed $K$? We will call this "Wisdom" to reflect the idea that it values appreciation of your uncertainty:
