...
In Chapter \@ref(illustratemodels), we worked with a set of theories. But what exactly do we mean by a theory---the term is used in very different ways in different social scientific contexts---and how does "theory" map onto a causal-model framework? 
...
We illustrate showing two models, $M^\prime$ and $M^{\prime\prime}$, that each imply a model $M$. In each case the lower level models contain additional nodes in a way that allows for a kind of "disaggregation" of exogenous nodes.
...
We start with the higher-level model, $M$, represented in Figure \@ref(fig:Highlow)(a). We can then offer the model, $M^\prime$, in panel (b) as a *theory*, or lower-level model, of $M$. We have added a node, $K$, in the causal chain between $X$ and $Y$. We can say that  $M^\prime$ is a *theory* of $M$ because the causal structure in $M^\prime$ *implies* the causal structure in $M$: if $X$ affects $Y$ through $K$, then $X$ affects $Y$. To put the point more intuitively, and closer in spirit to how social scientists commonly think about theory, we can think of  $M^\prime$ as an *explanation* of $M$. If we ask, "why does $X$ affect $Y$?", $M^\prime$ provides an answer: $X$ affects $Y$ because $X$ affects $K$, and $K$ affects $Y$.
...
## From structure to distributions over types

So far we have been discussing how a causal structure can be theorized, or justified, by a more detailed causal structure. That is, we have so far been dealing in structural causal models. A probabilistic causal model comprises both a causal structure, as represented in a DAG, and beliefs about distributions over the exogenous nodes --- in our setup, nodal types. In an $I \rightarrow D$ model, for instance, distributional beliefs include beliefs about how common positive effects of inequality on democracy are  (how common $\theta^D_{01}$ types are) or how common negative effects $\theta^D_{10}$ types are. Thus, we can also think about how a given set of beliefs about causal effects at a higher level might be supported by a lower-level model positing a set of beliefs about causal effects operating at that level.

For instance, suppose we start with a probabilistic causal model with the structure of Model $M$ and a belief that half of all $\theta^Y_{01}$

### A word of caution about priors across levels

We will often want to understand not just the mappings between nodal types across levels, but also the mappings between *beliefs* across levels. For instance, what does a given set of beliefs about how common different effects are in Model (b) imply about the prevalence of different effects in Model (a)? The mappings between beliefs across levels flow straightforwardly from the mappings across types that we have discussed. For instance, the proportion of positive $I \rightarrow D$ causal effects in Model (a) will be the sum of (i) the proportion of positive  $I \rightarrow M$ effects times the proportion of positive  $M \rightarrow D$ effects and (ii) the proportion of negative  $I \rightarrow M$ effects times the proportion of negative  $M \rightarrow D$ effects. 

The operation of these mappings is not always intuitive, however. To illustrate, suppose we begin with no information about causal effects in a model and so want to set flat priors, meaning that we accord equal prior weight to all nodal types at each node. In Model (a), flat priors would mean putting an 0.25 weight on each of the following: positive effects, negative effects, zero effects with $D$ always $0$, and negative effects with $D$ always $1$. Now, suppose we engage in the same flat-prior-setting in the lower-level model, Model (b). That is, we put equal weight (i.e., 0.25 across the board) on all nodal types at node $M$ and equal weight (0.25 across the board) on all nodel types at node $D$. Surprisingly perhaps, setting flat priors at this lower level in fact implies a strong weighting *against* either positive or negative causal effects at the *higher* level. Now, we are saying that the probability of a positive effect at the higher level is $(0.25 \times 0.25) + (0.25 \times 0.25) = 0.125$; and likewise for negative effects. 
...
So what kinds of elaborations---moves from a higher to a lower level---are allowable? What kinds of theories are consistent with a given higher-level model? 
...
### Quantifying the gains from a theory
...
As we will see in chapters to come, when we confront a causal model with data, we do not seek to confirm or falsify the model, but to update beliefs over its parameters. In particular, we will update beliefs about the (possibly joint) distribution of nodal types in the population of interest. 
