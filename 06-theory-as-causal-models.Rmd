...
In Chapter \@ref(illustratemodels), we worked with a set of theories. But we haven't yet been very explicit by what we mean by a theory or how  "theory" maps onto a causal-model framework. 
...
Structural models can imply multiple other simpler structural models. Similarly structural models can be implied by multiple more involved models. Take the very simple model, $M$, represented in Figure \@ref(fig:Highlow)(a). The model simply states that $X$ has (or *can* have) a causal effect on $Y$. 
...
A key idea, which we return to later, is that both $M'$ and $M''$ involve a redefinition of $\theta^Y$. That is we see a change in the endogenous nodes but these in turn imply a change in the interpretation of the exogenous nodes pointing into existing endogenous nodes (such as $Y$ in this example). 
...
### Probabilistic causal models and the integration of theory and data

A probabilistic model can also be implied by another probabilistic model, possibly coupled with data. 

Consider first implications without data. Causal Model (b) in Figure \@ref(fig:Highlow), coupled with a distribution over  $\theta^K$ and  $\theta^Y_{higher}$, produces a probabilistic causal model, $M_{lower}$. It in turn implies a probabilistic model, $M_{lower}$, formed from Causal Model (a) in Figure \@ref(fig:Highlow), and a *particular* distribution over $\theta^Y_{lower}$, that is, the distribution that preserves the causal relations implied by $M_{lower}$. Thus, for instance, the probability that $X$ has a positive effect on $Y$ in $M_{higher}$ is $\theta^{Y_{higher}}_{01}$; the probability that  $X$ has a positive effect on $Y$ in $M_{lower}$ is  $\theta^{M_{lower}}_{01}\theta^{Y_{lower}}_{01}  + \theta^{M_{lower}}_{10}\theta^{Y_{lower}}_{10}$.  COnsistency then requires that $\theta^{M_{lower}}_{01}\theta^{Y_{lower}}_{01}  + \theta^{M_{lower}}_{10}\theta^{Y_{lower}}_{10} = \theta^{Y_{higher}}_{01}$ and so the value of  $\theta^{Y_{higher}}_{01}$ is *implied* by $\theta^{M_{lower}}_{01},\theta^{Y_{lower}}_{01}, \theta^{M_{lower}}_{10},\theta^{Y_{lower}}_{10}$, but not vice versa.

A probabilisitc causal model coupled with data  implies  another model via Bayes rule.  For this reason we can fruitfully think of an initial model as being a *theory of* an updated model, coupled with data. Thought of in this way we have clarity over what is meant when we turn to theory to support a claim, but also what is meant when we seek to justify a theory. We might imagine a scholar arguing: "$M$: $X$ caused $Y$ in country $j$." When pushed for a justification for the claim they provide the lower level model: "$M:$ the average effect of $X$ on $Y$ in countries with feature $C=1$ is 0.95, making it likely that $X$ caused $Y$ in this case." Here $M$ is implied by $M$ coupled with data $C=1$. If pushed further as to why that theory is itself credible they might point to a lower level model consisting of structural model $X\rightarrow Y \leftarrow C$ plus flat priors coupled with data on $X,Y$ and $C$.  At each stage, as more justification is provided, the researcher formally provides lower level models.  
...
As the simplest illustration, we might imagine beginning with an $X\rightarrow Y$ model, $M_1$, in which, $X$ and $Y$ are binary, and we we believe that $Y$ possibly responds to $X$. If we have "flat" priors over causal types, in the sense described in chapter @\ref(bayes), then our prior uncertainty over the proposition that $X$ causes $Y$, under this model, is large; as is our uncertainty that $Y=1$ is dues to $X=1$ in a given case. In other words, given our theory, we are uncertain about the proposition $M_3$: $X$ caused $Y$. However if we then receive a lot of data, $D$, showing strong relations between $X$ and $Y$, then our updated model $M_2$, formed from combining $D$ and $M_1$ allows us to infer that  $X$ caused $Y$ in this case with greater certainty. 

Thus our new theory $M_2$ is (a) formally similar to $M_1$ (b) formed as a product of past theory plus evidence, here justified by $M_1$ given data $D$ and (c) capable of providing sharper implications than past theory.^[As a general matter an updated theory may not provider sharper claims for all queries. That is, in practice, posterio variance over queries can increase with more data. As a simple illustration; say  I start out thinking that  the probability that an outcome is due to A, B, or C is .9, .05, .05, respectively; I find evidence that convicnes me that A is not the cause, then I shift (a) to greater certainty about whether A was the cause but (b) greater uncertainty about whether B was the cause.]
...
Theories are “lower-level” causal models that explain or provide an account of a “higher-level”, simpler model. Two common forms of theorization involve disaggregation -- for instance by articulating  *mediating* and *moderating* relationships---and generalization:
...
<!-- AJ: Have deleted the words "TEXT" and the paragraph that follows -- the runningsubs figure doesn't seem to be here anymore.  -->
...
We begin with theory $M_1$ which together with data $D$ implies claim $M_2$. We then posit theory $M_0$ of $M_1$---and so $M_0$ implies $M_1$, but $M_0$, coupled with $D$, produces a claim , $M_2'$, that is different---and hopefully better---than $M_2'$.

We  illustrate using a theory that allows use of the "front door criterion." 

Imagine we have data on three variables, $X$, $Y$, and $M$. We begin, however, with a model $M_1$: $A \rightarrow X \rightarrow Y \leftarrow A$, that includes nodes for two of the three variables, $X$ and $Y$, but not $M$, as well as a node $A$ for which we do not have data. 

We see a strong correlation between $X$ and $Y$ and infer $M_2$: that $X$ is a likely cause $Y$. Our inference under $M_2$ is uncertaing because we are aware that the correlation may be due to the confound $C$.

We posit  theory $M_0$ $A \rightarrow X \rightarrow M \rightarrow Y \leftarrow A$. This theory now lets us make better use of data $M$. If we observe for instance that $X$ and $M$ are uncorrelated, then we infer with confidence that in fact $X$ did not cause $Y$, despite the correlation. 

Thus, in return for specifying a theory for $M_1$, we have been able to make better use of data and form a more confident conclusion. Critically the theory does not alter *priors* over our query. Our prior over the effect of $X$ on $Y$ may be identical under $M_1$ and $M_0$, but our conclusions, given data, differ.
...
This one reason to theorize our models --- develop more elaborate, lower-level models to support them --- is to be able to reap greater inferential leverage from the more elaborate theory. But how much more? For instance, what are the gains from a theory that introduces a node $K$ relative to one that does not include $K$? What is the value-added of the more elaborate theory? 
...
Another approach is to ask: how much better are our guesses having observed $K$ compared to what we would have guessed before, *given* what we know having observed $K$?
...
This captures how much better off we are with the guess we have made given current data ($q_k$) compared to the guess we would have made if we had a theory that did not let us make use of it  ($q_0$), knowing what we know having observe $K$ ($p(q|k)$. 

A modest advantage of this conceptualization is that we can still record gains in learning even if the learning operates such that the posterior variance is larger than the prior variance.  Even so, the implications for strategy are the same since wisdom is maximized by a strategy that reduces expected squared error.  -->
...
### Mappings are not 1 to 1

The mappings between higher-level claims and theories may not be one-to-one. A single theory can support multiple higher-level models. Moreover, a single higher-level relation can be supported by multiple, possibly incompatible lower-level theories. 
...
It is also true that any given theory logically implies multiple *higher-level* claims about causal relations. $L_2$ implies both $H_2$ and $H_3$. Note, however, that the multiple higher-level claims that follow from a theory *must* be compatible with one another. 
...
## Application: Formal theories and causal models

In social sciences there sometimes seems to be a sharp divide between formal---analytic--theories and empirical claims.

Consider for instance the claim that the number of Nash equilibria is generically odd in finite games is often understood to be a theoretical claim. Unless there are errors in the derivation of the result, the claim is true in the sense that the conclusions follow from the assumptions. There is no evidence that we could go looking for in the world to assess the claim. The same can be said of the theoretical claims of many formal models in social sciences; they are theoretical deductions of the if-then variety [@clarke2012model]. 



In the standard ("hypothetico-deductive") model; a theory originates in the mind and then retained or rejected if it is consistent with data [@popper2014conjectures]. In practice, this  usually requires a step in which the formal theory is translated either wholesale or piecemeal into a set of empirical statements which are put to the test.  We seek to confirm theories by developing and testing hypotheses about the similarity of a model or theory to particular features of the world. In both cases, a theory is posited---possibly on the basis of logic or background knowledge---and then assessed. The value (truth or usefulness) of the model itself is the object of inquiry. [@clarke2012model]

The causal modelling framework allows for a different approach to combining theory and data, in which theories  are represented as causal models that capture the ideal operation of the theory as well as the conditions under which the claims of the theory are expected to hold. The causal model can naturally include uncertainty about any or all assumptions and claims of the theory, allowing for an updating of the theory along with data. These are what might be called, at the risk of provoking ire, empirical theories. Theory, in this sense, is a collection of claims with *empirical* content: a theory refers to a set of proposition of causal relations in the world that might or might not hold, and is susceptible to assessment against data.

Though it is not standard practice, formal models produced by game theorists can often be represented using the notation of structural causal models. Moreoever, doing so may be fruitful. 
...
First the discussion highlights that thinking of theory as causal models does not force a move away from abstract analytic theories; these can often be incorporated in the same framework. This is true even for equilibrium analysis that seems to involve a kind of simultaneity on first blush. 
...
Third the example shows a way of building a bridge from pure theory to empirical claims. One can think of Nash's theory as an entirely data free set of claims. When interpreted however as a proposition about the ways players *might* behave, represented as a causal model, we are on a path to being able to use data to refine the theory. Thus we might begin with a formal specification like that in Figure \@ref(fig:nfg5) but with initial uncertainty about player rationality, optimizing behavior, and equilibrium selection. This theory nests Nash but does not presume the theory to be a valid description of processes in the world. Combined with data, however, we shift to a  more refined theory that selects Nash from the lower level model. 



## Conclusion {#theory_conclusion}

We see three gains from this conceptualization of theory. First it helps clarify what is required for theoretical justification of a model. Second it clarifies what is meant by explanation. Third it provides one  framework to answer the question: "when is a theory useful?"  
