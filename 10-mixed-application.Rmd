...
We can undertake the same exercise for a negative causal effect, generating estimates for a comparable set of estimands, as shown in Figure \@ref(fig:mixedhist2). Here we are asking for what proportion of cases overall inequality has a negative effect on democratization; for what proportion of $I=1, D=0$ cases inequality prevented democratization; and this latter query conditional on different clue realizations. 
...
We might wonder, at the same time, how much we are in fact learning from the data, as compared to what we built into the model at the outset, including through the monotonicity restrictions that we imposed. To examine this, in Figures \@ref(fig:dcaused) and \@ref(fig:dprevented), we plot the mean (and one-standard-deviation error bars) of our prior and our posterior on the same set of queries. We see that there is almost no shift in beliefs for the positive-effect queries, with a somewhat greater shift in means for the negative-effect queries. However, our uncertainty remains high even in for negative effects, though has shrunk some. By comparing the estimates given $M=1$ to those given $M=0$, we can also see that, in this example, we do not happen to learn anything from the data about $M$'s probative value: the change in beliefs between observing $M=1$ and observing $M=0$ remains about the same in our posteriors as it was in our priors. 
