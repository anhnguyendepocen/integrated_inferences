# Integrated Inferences Application {#mixingapp}
...
Instead of positing a belief over the nodal types for a given case, $\theta$, we now need to posit a belief over the *distribution* of nodal types---that is, over $\lambda$. Whereas in the simple process-tracing model we *specified* the shares of nodal types for $M$ (for instance), we now specify a prior *distribution* over the nodal type shares. ,  We do the same, of course, for all nodes. Because we set a prior distribution over nodal types (rather than fixed proportions), we can now update on these population-level distributions as the model confronts data. 
...
kabble(head(data), caption = "Data (snapshot) derived from Haggard and Kaufman (2012)", booktabs = TRUE)
...
kabble(cor(data[,-1], use = "pairwise.complete.obs"), digits = 3,
      caption = "Pairwise correlated in the democracy-inequality (PIMD) data. P = Pressure, I = Inequality, M = Mobilization, D = Democratization")
```
...
if(do_diagnosis){
  ...
  write_rds(model, "saved/10_PIMD_updated_2.rds")
  }

model <- read_rds("saved/10_PIMD_updated_2.rds")
...
I0D1_Cases <- cases_table(
  conditional_inferences(
    model, using = "posteriors", 
  query = "D[I=1] < D[I=0]",   given = "D==1 & I==0"), 
  case_names  = c("Mexico (2000)", "Taiwan (1996)", 
                  "Albania (1991)", "Nicaragua (1984)"))

kabble(I0D1_Cases, digits = 3, caption = "Four cases with low inequality and  democratization. Question of interest: Was low inequality a cause of democracy? Table shows posterior beliefs for different data for four cases given information on $M$ or $P$. Data from Haggard and Kaufman (2012). Analyses here use a model with assumptions on monotonic effects but magnitudes of effects updated from data.")  %>%  kable_styling(latex_options = "scale_down")

```

In Table \@ref(tab:HK10cases1) we ask what we would now infer about cases given democratization and *high* inequality and different observations on mobilization and international pressure. This table is the analogue of Table \@ref(tab:HK8cases2) in Chapter \@ref(ptapp).
...
kabble(I1D1_Cases, caption = "Four cases with high inequality and  democratization. Question of interest: Was high inequality a cause of democratization? Table shows posterior beliefs for different data for 4 cases given information on $M$ or $P$. Data from Haggard and Kaufman (2012).  Analyses here use a model with assumptions on monotonic effects but magnitudes of effects updated from data.r")  %>%  kable_styling(latex_options = "scale_down")
...
kabble(dplyr::select(I0D0, P,M, posterior),digits = 3, caption = "No inequality and No democratization: Was no inequality a cause of no democratization? Analyses here use priors assuming only monotonic effects.")
```
...
kabble(dplyr::select(I1D0, P,M, posterior), digits = 3, caption = "Inequality and No democratization: Was inequality a cause of no democratization? Analyses here use priors assuming only monotonic effects.")
```
...
write_rds(pimd_queries_2, "saved/10_pimdqueries_2c.rds")
}

pimd_queries_2 <- read_rds("saved/10_pimdqueries_2c.rds")

pimd_queries_2 %>% select(Given, mean) %>% kabble(digits = 2, caption = "Probability that inequality increases chances of democratization given:")

```

In figure \@ref(fig:mixedhist), we graph the priors for each of these queries next to the posteriors.

```{r pimdqueries, include = FALSE, echo = FALSE, message = FALSE, warning = FALSE}
...
```{r mixedhist, echo =FALSE, fig.cap = "Posteriors on Causes of Democratization", warning = FALSE,message=FALSE}
...
```{r pimdnegqueries, include = FALSE, warning = FALSE, message = FALSE}
...
```{r mixedhist2, echo =FALSE, fig.cap = "Posteriors on Causes of Democratization", warning = FALSE, message=FALSE}

read_rds("saved/10_pimdnegqueries.rds") %>%
