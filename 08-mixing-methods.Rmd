...
To illustrate, consider the following situation. Suppose that our model includes a binary treatment $X$ that is assigned to 1 with probability 0.5; an outcome, $Y$; and a third "clue" variable, $K$, all observable. We allow for $X$ to affect $Y$. In addition, we posit an unobserved variable $\theta$, representing $Y$'s nodal type, with $\theta$ taking on values in $\{a,b,c,d\}$ with equal probability. (We interpret the types in $\{a,b,c,d\}$ as defined in Section \@ref(counterfactualmodel).) In addition to pointing into $Y$, moreover, $\theta$ affects $K$. In particular, $K=1$ whenever $X$ has an effect on $Y$, while $K=1$ with a 50% probability otherwise. In other words, our clue $K$ is informative about a unit's nodal type for $Y$. As familiar from Chapters \@ref(pt) and \@ref(ptapp), when we observe $K$ in a case we can update on causal effects within the case since that $K$ value will have different likelihoods under different values of $\theta$.
...
* $\Theta$ is now a vector with two elements that can take on one of 16 values $(a,a), (a,b),\dots (d,d)$ as determined by $\lambda_\theta$
* $\mathbf Y$ is a vector that reflects the elements of $\Theta$ and $\mathbf X$ in the obvious way (e.g., $\mathbf X=(0,0), \Theta=(a,b)$ generates outcomes $\mathbf Y=(1,0)$)
* The vector $\mathbf K$ has the same domain as $\mathbf X$ and $\mathbf Y$, and element $\mathbf K[j]=1$ if $\theta[j]=b$.

Now, consider a causal estimand. In a single-case setup, we might ask whether $X$ has an effect on $Y$ in the case. For a multi-case setup, we might ask what the Sample Average Treatment Effect, $\tau$, is. Note a subtle difference in the nature of the answers we seek in these two situations. In the first (single-case) instance, our estimand is binary---of the form: "is the case a $b$ type?"--and our answer is a probability. In the multi-case estimation of the SATE, our estimand is categorical and our answer is a distributio: "what is the probability the $\tau$ is 0", "what is the probability the $\tau$ is .5", and so on).
...
While the estimand shifts, we can use the tools introduced for single-case process tracing in Chapters \@ref(pt) and \@ref(ptapp) to analyze this (superficially) multi-case study. To begin, our prior on the probability that $\tau=1$ is the prior that $X$ has a positive effect on $Y$ in both cases, that is, that $\Theta = (b,b)$: just 1 in 16. 
...
3. **Causal types**. A full set of nodal types gives rise to a full set of causal types, encompassing all possible combinations of nodal types across all nodes in the model. We let $\theta$ denote an arbitrary causal type. For instance $X \rightarrow M \rightarrow Y$ we might have type $\theta = (\theta^X_1, \theta^M_{01}, \theta^M_{01})$.
...
2. **Event probabilities**. Each causal type in turn implies a data type. For instance $\theta = (\theta^X_1, \theta^M_{01}, \theta^Y_{01})$ implies data $X=1, M=1, Y=1$. Let $D(\theta)$ denote the data type implied by causal type $\theta$. We then use $\Theta(d)$ to denote the set of causal types that imply a given data type:
...
$$
p(d|\lambda) = 
  \text{Multinom}\left(n_{XYK}|\sum n_{XYK}, w_{XYK}(\lambda)\right)  \\
$$
...
4. **Estimation**. We now have all the components for updating on $\lambda$. Applying Bayes rule from Chapter @\ref(bayeschapter) we have:
...
head() %>% kable(digits = 2, caption = "An illustration of a posterior distribution for the Chain model. Each row is a draw from $p(\\lambda|d))$")
