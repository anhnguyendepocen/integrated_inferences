...
* We have a treatment vector, $\mathbf X$, that can take on one of four values, $(0,0), (0,1), (1,0), (1,1)$ with equal probability. 
...
While the core conceptual logic is the same for multi-case and single-case inference, we operationalize these problems somewhat differently. 
...
Let $\tau^{|x}$ denote a query of interest---where the query is conditional in the sense that it relates to cases with data $x$. Let $q^i_j$ denote the probability that $\tau^{|x} = \tau_j^{|x}$ when $\lambda = \lambda^i$ for a case with data $X=x$.  Note $q^i_j$  can be written $z^i/d^i$ where $z^i_j = \Pr(\tau^{|x} = \tau^{|x}_j, X=x | \lambda^i)$. 

To illustrate say in an $X\rightarrow Y$ model we were interested the effect of $X$ on $Y$ in a case with $X=1, Y=1$. Then $d^i = (\lambda^i)^X_1((\lambda^i)^Y_{01} + (\lambda^i)^Y_{11})$. If $z^i_j =  (\lambda^i)^X_1((\lambda^i)^Y_{01})$ for $\tau^{|x}_j = 1$, and so $q_j = \frac{(\lambda^i)^Y_{01}}{(\lambda^i)^Y_{01} + (\lambda^i)^Y_{11}}$
...
Note however that the *posterior* distribution on $\lambda^i$ given observation of $x$ in the $S^0$ set is $\frac{p^id^i}{\sum_k p^kd^k}$ and that $p^iz^i_j = p^id^iq^i$. Therefore this quantity can also be interpreted as the posterior mean of $q^i$, after observing both $S^0$ and $S^1$. 
...
We do not need data on all nodes in order to implement the procedure. If we have data only one some of the nodes in a model, we follow the same basic logic as with partial process-tracing data. In calculating the probability of a pattern of partial data, we use all columns (data types) in the ambiguity matrix that are consistent with the partial data. 
...
```{r measurementerror2, echo = FALSE}
set.seed(1)
