...
To illustrate, consider the following situation. We study two units drawn from some population, with a binary treatment $X$ assigned with probability 0.5, independently for each unit, and an outcome $Y$ along with clue variable $K$, all observable.  We suppose that $X$ can affect $Y$. In addition there is a background, unobserved, variable $\theta$ (causal type) that takes on values in $\{a,b,c,d\}$ with equal probability and that affects both $K$ and $Y$. (We interpret $\{a,b,c,d\}$ as introduced in Section \@ref(counterfactualmodel)). In particular we suppose that in any given case $K=1$ with certainty whenever $X$ causes $Y$, and that $K=1$ with a 50% probability otherwise. Thus, $K$ is informative about a unit's causal type. 

Note that we have described the problem at the unit level. However we can redescribe it for the two-case setup thus:
...
We now have a representation that maps directly onto the case-level structures used in Chapters \@ref(pt) and \@ref(ptapp) and can use the tools introduced there to analyze this seemingly multi-case study. 
...
Thus if observe data pattern $K = (0,1)$, the probability of this pattern for type vector $(b,b)$ ($\tau = 1$)  is 0. For the type vectors $(d,d), (b, d), (d,b)$ it is $.25, 0, .5$, respectively.  The table below shows the posterior distribution on the average treatment effects given different $K$ patterns observed.  
