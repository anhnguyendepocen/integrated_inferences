...
The `CausalQueries` package, set up to work with binary nodes, can be used to represent non-binary data as well. The trick, as it were, is to express integers in base-2 and then represent the integer as a series of 0's and 1's on multiple nodes. In base-2 counting, the rightmost digit represents $2^0$ (or 1), the digit to its left represents $2^1$, the digit to left of that represents $2^2$, and so on. All digits are binary, taking on either 0 or 1 values. To represent 4 values, for instance, we need only 2 digits: $00, 01, 10, 11$ represent $0, 1, 2, 3$ in base-10 counting. 

Thus, if we want $X$ and $Y$ to each have 4 values, we represent $X$ via two separate binary nodes, $X_1$ and $X_2$, which serve as placeholders for the two required digits. These two nodes can take on the combinations $0,0; 0,1; 1,0; 1,1$, mapping into $X$'s four values. So, for instance, $X_1 = 0, X_2 = 1$ can represent $X=1$, and $X_1 = 1, X_2 = 1$ can represent $X=3$.  all possible values on a model with $Y_1 \leftarrow X_1 \rightarrow Y_2, Y_1 \leftarrow X_2 \rightarrow Y_2$. Thus the information on $X$ and $Y$ gets fully recorded on 4 nodes rather than 2: non binary data $X=3, Y=2$ gets represented with binary data $X_1=1, X_2=1, Y_1=1, Y_2 = 0$. Note that both $X_1$ and $X_2$ point into each of  $Y_1$ and $Y_2$, resulting in $2\times 2\times 16 \times 16 = 1024$ causal types, as required. To allow for the full range of nodal types we need to allow a joint distribution over $\theta^X_1$ and $\theta^X_2$ and over  $\theta^Y_1$ and $\theta^Y_2$ which results in 3 degrees of freedom for $X$ and 255 for $Y$, as required.
