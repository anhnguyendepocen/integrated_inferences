...
The `CausalQueries` package, set up to work with binary nodes, can be used to represent non-binary data as well. The trick, as it were, is to express integers in base-2 and then represent the integer as a series of 0's and 1's on multiple nodes. In base-2 counting we would need to represent four integers $00, 01, 10, 11$. These are two digit numbers. If we use one binary node, $X_1$ to represent the first digit, and a second node $X_2$ to represent the second, we have enough information to capture the four values: $X_1 = 0, X_2 = 0$ represents $X=0$, $X_1 = 0, X_2 = 1$ represents $X=1$,   $X_1 = 1, X_2 = 0$ represents $X=2$, and $X_1 = 1, X_2 = 1$ represents $X=3$. We construct $Y$ in the same way and then represent a simple $X \rightarrow Y$ relation as a model with both $X$ nodes pointing into both $Y$ nodes: $Y_1 \leftarrow X_1 \rightarrow Y_2, Y_1 \leftarrow X_2 \rightarrow Y_2$. To allow for the full range of nodal types we need to allow a joint distribution over $\theta^{X_1}$ and $\theta^{X_2}$ and over $\theta^{Y_1}$ and $\theta^{Y_2}$, which results in 3 degrees of freedom for $X$ and 255 for $Y$, as required.
