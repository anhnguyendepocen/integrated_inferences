...
pimd_nonmon %>%
   update_model(data, keep_fit = TRUE, iter = 2000) %>%
...
In our model we assumed that $M$ is monotonic in $I$. How much do conclusions depend on this?  We answer the question by comparing our conclusion to what we woudl conclude withiout this assumption. Our answers depend on the quantity examined.

We first show results for population inference from a mixed methods analysis. As seen in Table XX our inferences regarging the overall effect of $I$ on $D$ are not very sensitive to this assumption. Our conditional inferences are however. In particular, in cases with $I=0, D=1$ we are more likley to think that democratization was due to *low* inequality given the  unrestricted model. When we see that in fact there was no mobilization, our attribution increases in the restricted model but decreases in the unrestricted model. 

AJ: Check intuition here!

In the non-monotonic model you entertain the possibility that low inequality mattered not just directly but also, perhaps, by inducing protests. However, when you observe no protests, you rule out this possible pathway. In the monotonic model you do not think that democratization could have been produced by low inequality via demonstrations---but nevertheless entertain the possibility of mobilization that is *not* due to inequality, which could nevertheless be the cause of democratization. In this case, observing no mobilization removes a rival, cause of democratization, not a second channel. 
...
if(do_diagnosis){
  ...
  q_ate_id  <- query_model(pimd, "D[I=1] - D[I=0]", using = "posteriors", 
                           given = c(TRUE, "I==0 & D==1", "I==0 & M==0 & D==1"))
  q_ate_id2 <- query_model(pimd_negmed, "D[I=1] - D[I=0]", using = "posteriors", 
                           given = c(TRUE, "I==0 & D==1", "I==0 & M==0 & D==1"))

  out <- cbind(q_ate_id, q_ate_id2[, c("mean", "sd")])
  names(out) <- c("q", "Given", "u", "Mean (M1)", "sd (M1)", "Mean (M2)", "sd (M2)") 
