...
<!-- ALSO DO WITH CONFOUNDING X<-->Y X->M->Y -->
...
with(CausalQueries::democracy_data, lm(P ~ I)) %>% summary()

with(CausalQueries::democracy_data, lm(P ~ M)) %>% summary()
...
if(do_diagnosis){
  ...
  get_loo_likelihood(CausalQueries::democracy_data, pimd, 1000) %>% write_rds("saved/ch15_loo_pimd.rds")
    
  get_loo_likelihood(CausalQueries::democracy_data, pimd_negmed, 1000) %>% write_rds("saved/ch15_loo_pimdnegmed.rds")
