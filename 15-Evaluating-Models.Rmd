...
par(mfrow = c(1,2))
  plot(loo_restricted$case_likelihood, loo_restricted$data$count, 
       xlab =  "Case likelihood  | restricted model", ylab =  "Actual number of cases", type = "n")
text(loo_restricted$case_likelihood, loo_restricted$data$count, loo_restricted$data$event)

  plot(loo_unrestricted$case_likelihood, loo_unrestricted$data$count, 
       xlab =  "Case likelihood  | unrestricted model", ylab =  "Actual number of cases", type = "n")
text(loo_unrestricted$case_likelihood, loo_unrestricted$data$count, loo_unrestricted$data$event)
...
par(mfrow = c(1,2))
  plot(loo_restricted_2$case_likelihood, loo_restricted_2$data$count, 
       xlab =  "Case likelihood  | restricted model", ylab =  "Actual number of cases", type = "n")
text(loo_restricted_2$case_likelihood, loo_restricted_2$data$count, loo_restricted$data$event)

  plot(loo_unrestricted_2$case_likelihood, loo_unrestricted_2$data$count, 
       xlab =  "Case likelihood  | unrestricted model", ylab =  "Actual number of cases", type = "n")
text(loo_unrestricted_2$case_likelihood, loo_unrestricted_2$data$count, loo_unrestricted_2$data$event)
...
if(do_diagnosis){
 ...
 data <- CausalQueries::democracy_data 
     write_rds(data, "saved/ch15_pimddata.rds") 
...
data               <-   read_rds("saved/ch15_pimddata.rds") 
...
if(do_diagnosis){
 ...
 write_rds(data.frame(pimd_ateID = ate_id),
           "saved/ch15_ateID.rds")
...
hist(ateid$pimd_ateID, xlim = c(-1, .6), main = "Posterior Distribution from PIMD Model", xlab = "Effect of I on D") 
...
if(do_diagnosis){
  ...
  get_loo_likelihood(CausalQueries::democracy_data, pimd, 20) %>% write_rds("saved/ch15_loo_pimd.rds")
    
  get_loo_likelihood(CausalQueries::democracy_data, pimd_negmed, 20) %>% write_rds("saved/ch15_loo_pimdnegmed.rds")
...
plot(loo_pimd$case_likelihood, loo_pimdnegmed$case_likelihood, xlab = "likelihood under monotonic model", ylab = "likelihood under unrestricted model", type = "n")
text(loo_pimd$case_likelihood, loo_pimdnegmed$case_likelihood, loo_pimd$data$event, cex = .7)
abline(a = 0, b = 1, col = "red")

#Is last term above correct?

par(mfrow = c(1,2))
  plot(loo_pimd$data$count, loo_pimd$case_likelihood, col = "red")
  text(loo_pimd$case_likelihood, loo_pimdnegmed$case_likelihood, loo_pimd$data$event)
  
  plot(loo_pimdnegmed$data$count, loo_pimdnegmed$case_likelihood)

with(loo_pimd,
     {
  plot(loo_pimd$data$count, case_likelihood, col = "red")
  text(case_likelihood, case_likelihood, loo_pimd$data$event)
     }
)
