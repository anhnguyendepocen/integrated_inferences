...
One approach to cross-validation is the "leave-one-out" (LOO) algorithm. In a LOO approach, we update the model using all data points except for one; ask how well the model performs in predicting the left-out observation; repeat for every data point in the dataset; and aggrgeate predictive performance across all iterations. In the standard implementation, the measure of predictive success in each iteration is the residual error, typically on an outcome variable. In a CausalQuery

COMPARE ONE TO ANOTHER IN TERMS OF LOO
 3d7572d6abe89f9a89bd4de4d8cc1f1aa4ef5c9b
...
The LOO estimate of out-of sample predictive fit:

$$\prod_1^np(y_i|y_{-i}, \text{model})$$

likelihood of the data under the restricted model is `r  formatC(loo_restricted$loo_likelihood, format = "e", digits = 2)`
and under the unrestricted models:  `r  formatC(loo_unrestricted$loo_likelihood, format = "e", digits = 2)`. An improvement on the order of   `r  formatC(loo_unrestricted$loo_likelihood/loo_restricted$loo_likelihood, format = "e", digits = 2)`. 
