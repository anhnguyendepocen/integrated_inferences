...
hist(replicates_restricted$Y, 
     main = paste("Y mean | stronger: p = ", round(mean(replicates_restricted$Y > mean(data$Y)),2)),
...
More formally, we calculate the LOO likelihood for each model as `formatC(loo_pimd$loo_likelihood, format = "e", digits = 2)` for the model with monontonicity assumptions throughout, `formatC(loo_pimdnegmed$loo_likelihood, format = "e", digits = 2)` for the model that relaxes the montonocity assumption for $I \rightarrow M$, and `formatC(loo_pimdnonmon$loo_likelihood, format = "e", digits = 2)` for the model with no monontonicity assumptions at all. In other words, we see that the most restrictive model performs best, by about an order of magnitude, followed by the least restrictive model, and then the moderately restrictive one.
