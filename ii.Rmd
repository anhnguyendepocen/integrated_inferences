...
C_Y1_only <- 
  make_possible_data(model, given = given, vars = "M", N = c(1,1), 
                     condition = c("X==0 & Y==1", "X==1 & Y==1"), prefix = "C")

D_X1_only <- 
  make_possible_data(model, given = given, vars = "M", N = c(1,1), 
                     condition = c("X==1 & Y==0", "X==1 & Y==1"), prefix = "D")

E_at_random <- 
  make_possible_data(model, given = given, vars = "M", N = 2, prefix = "E")

```

We consider a situation in which one has access to $X,Y$ data of the form:

```{r, echo = FALSE}
kable(given)
```

and one assesses four strategies. Strategy $A$ choses two cases on the regression line (one in the X=Y=0 cell and onne in the X=Y=1 cell); strategy $B$ chooses off the regression line and straregy C selects on the dependent variable -- choosing cases with $Y = 1$.

Different strategies $A - D$ can each yield up to four different data types. Each one of these is likely to arise with a different probabilities and is associated with a different inference. This allows is to assess the *expected* posterior variance associated with each strategy. We exclude foor the moment the random strategy which produces up to 30 different data types given the available cases. 
...
df <- A_on_regression_line %>% 
  ...
  merge(C_Y1_only, by = "event", all = TRUE) %>%
  merge(D_X1_only, by = "event", all = TRUE) %>%
  dplyr:::mutate_if(is.numeric, ~replace(., is.na(.), 0))

table(duplicated(t(df)))
names(df) <- c("Event", paste0(rep(c("A", "B", "C", "D"), each = 4), 1:4))
```
...
if(do_diagnosis){
 ...
 strategies <- 
   list(A_on_regression_line, B_off_regression_line, C_Y1_only, D_X1_only)
...
# Set the reference model that we will use to calculate data probabiities. The reference model is the original model updated with the XY data we've already seen.
 ...
 probabilities <- lapply(strategies, function(s) 
    average_data_probabilities(reference_model, s, using = "posteriors")
    )
  ...
  df <- rbind(df, 
              c("Probability", round(unlist(probabilities),2)),
              c("Posterior variance",   round(unlist(lapply(estimates_db, function(j)   unlist(j[,5]))),2)))
  kable(df, caption = "Each colum shows a possible distribution of data that can be generated from a given strategy. Each strategy generates one of four different data patterns. We calcualte the probability of each, given the data seen so far, and the posterior variance given each data realization.")
```

Thus each strategy generates one of four different data patterns. We calcualte the probability of each, given the data seen so far, and the posterior variance given each data realization.  The implied *expected* variance under each strategy is summarized below:
```{r, echo = FALSE}
expected_posterior_var <- sapply(0:3, function(i) sum(
 as.numeric(df[df$Event == "Probability", (i*4+2):(i*4 + 5)])*
 as.numeric(df[df$Event == "Posterior variance", (i*4+2):(i*4 + 5)]))
 )
kable(data.frame(Strategy = c("A", "B", "C", "D"), Variance = expected_posterior_var))
```
...
```{r, include = FALSE}  
  ...
  # Calculate the probability of each data type given the reference model.  
  # illustration_diagnosis <- 
  #   diagnose_strategies(
  #       reference_model = reference_model, 
  #       analysis_model  = model,
  #       given = given, 
  #       queries = "Y[X=1] > Y[X=0]",
  #       estimates_database = estimates_dbs[[1]],
  #       possible_data = strategies[[1]])
  # 
...
```{r showx1y1diagnosestrategies, eval=FALSE, include = FALSE}
diagnosis_X1Y1 <- 
  diagnose_strategies(
    reference_model = reference_model, 
    analysis_model  = model,
...
We now apply the procedure systematically to a set of strategies for multiple models and given differnet types of pre-existing data. 
