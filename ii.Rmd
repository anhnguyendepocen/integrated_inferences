...
```{r ch11define_running_model_1, warning = FALSE}
model <- 
...
model_priors <- 
 ...
 set_priors(node = "S", statement = "S==1", alphas = 10) %>%
...
#         set_priors(alphas = list(M = c(`(M[I=0] > M[I=1])` = 0.5)))  #Set less weight on negative effect of I on M
           ...
           set_parameters(node = "M", statement = "(M[I=0] > M[I=1])", alphas = 0.125)  #Set less weight on negative effect of I on M
...
```{r table_inferences_priors_effect, echo = FALSE}
...
```{r pimdlearn_priors_effect, echo = FALSE, message = FALSE, warning = FALSE}
...
```{r ch11define_running_model_3, warning = FALSE}
...
## DYNAMIC STRATGIEZ WITH CONFOUNDED MODEL IS NOT BEING DONE (SEE EVAL = FALSE ABOVE)
...
Ch12_base_model_2_regr <- read_rds("saved/Ch12_base_model_2_regr.rds")
Ch12_base_model_2_flat <- read_rds("saved/Ch12_base_model_2_flat.rds")
df <- rbind(data.frame(given = "regr", M_base_model_2_regr$diagnoses_df),
            data.frame(given = "flat", M_base_model_2_flat$diagnoses_df))
...
## Simple walks through

### Learning from mediators on and off the regression line for an X->M->Y model 
...
observed <-  collapse_data(data, model, remove_family = TRUE)
...
if(do_diagnosis){
  ...
  write_rds(update_model(reference_model, data, fit = fit), "saved/ch15_ref_update.rds") 
  write_rds(update_model(analyst_model, data, fit = fit), "saved/ch15_ana_update.rds") 
  
  replicates_ana <- 
  t(replicate(sims, make_data(ana_update, n = nrow(data), param_type = "posterior_draw")$Y))
  write_rds(replicates_ana, "saved/ch15_replicates_ana.rds") 

  replicates_ref <- 
  t(replicate(sims, make_data(ref_update, n = nrow(data), param_type = "posterior_draw")$Y))
  write_rds(replicates_ref, "saved/ch15_replicates_ref.rds") 

  }

  ref_update     <- read_rds("saved/ch15_ref_update.rds") 
  ana_update     <- read_rds("saved/ch15_ana_update.rds") 
  replicates_ana <- read_rds("saved/ch15_replicates_ana.rds") 
  replicates_ref <- read_rds("saved/ch15_replicates_ref.rds") 
