...
# Learning from a moderators on and off teh regression line for an X->M->Y model 
...
test <- 
  diagnose_strategies(
    ...
    given = gen_given(4,2,2,4),
    queries = list(ATE = "Y[X=1]-Y[X=0]", Some_effect = "Y[X=1]!=Y[X=0]", ProbPos = "Y[X=1]>Y[X=0]"),
    subsets = list(TRUE, TRUE, "X==1 & Y==1"),
    sims = 8000,
    fit = fit, 
    refresh = 1000,
    iter = 8000)
