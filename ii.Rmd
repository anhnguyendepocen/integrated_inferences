...
We illustrate showing two models, $M^\prime$, $M^{\prime\prime}$ that each imply a model  $M$. In each case the lower level models contain additional nodes in a way that allows for  a kind of "disaggregation"  of exogneous nodes.
...
Critically notice that since lower level models imply higher level models we think of theories as implying the models they are theorizing. If you believe Model $M'$, then you also must believe Model $M$. If it is possible that $X$ can affect $K$ and possible that $K$ can affect $Y$ then it is possible that $X$ can affect $Y$. The coverse is not true however s you might think it possible that $X$ can effect $Y$ even if you think it not possible that $X$ can affect $K$. Similarly, if you believe Model (c), then you must also believe Model (a): if it is true that $X$ can affect $Y$, possibly in ways that are moderated by $C$, then it is trivially true, more simply, that $X$ can affect $Y$. 
...
And we allow for $M$ to have a positive, negative, or no effect on $D$, with $\theta^D_{\text{lower}}$'s possible values again being one of four nodal types ($\theta_{10}^D$, $\theta_{01}^D$, $\theta_{00}^D$, $\theta_{11}^D$).
...
In Table \@ref(tab:PO16b) we give a mapping from a subset of these lower level types to the upper level types corresponding to the model in (a). Here we use a  notation $\theta_{ij}^{gh}$ where the  subscripts and superscripts represent $Y$'s potential outcomes for different values of $I$ and $E$.^[To aid interpretation, we always place $I$ on the horizontal axis and a second variable (here, $E$) on the vertical access. In other words, the value of $I$ increases from 0 to 1 as we move to the _right_, from $i$ to $j$ or from $g$ to $h$. And the value of $E$ increases from 0 to 1 as we move _up_, from $i$ to $g$ or from $j$ to $h$. $\theta_{ij}^{gh}$ thus denotes a unit that has outcome $i$ when $I=0, E=0$, $j$ when $I=1, E=0$, $g$ when $I=0, E=1$, $h$ when $I=1, E=1$.]



----------------------------------------------------------------------------------------------------------------------------
   Low Type          if $I=E=0$   if $I=0,E=1$  if $I=1,E=0$   if $I=E=1$   High Type
-------------------  -----------  ------------  -------------  -----------  ------------------------------------------------
$\theta^{00}_{00}$     0           0             0             0             $\theta^D_{00}$

      2                 0           0             0            1             $\theta^D_{00}$ if $E=0$, else $\theta^D_{01}$ 

      3                 0           0             1            0             $\theta^D_{01}$ if $E=0$, else $\theta^D_{10}$

      4                 0           0             1            1             $\theta^D_{01}$

  $\vdots$             $\vdots$     $\vdots$      $\vdots$     $\vdots$      $\vdots$     

      15               1           1             1             0             $\theta^D_{11}$ if $E=0$, else $\theta^D_{10}$

$\theta^{11}_{11}$     1           1             1             1             $\theta^D_{11}$
-----------------------------------------------------------------------------------------------------------------------------
Table: (\#tab:PO16b) With two binary causal variables, there are 16 causal types: 16 ways in which $Y$ might respond to changes in the two variables.
...
Importantly we see that the mapping between lower- and higher-level types can depend on data, that is, on the value of the moderator. More generally we can also think of the value of exogeneous nodes, $E$ and $I$, as being nodal types for those nodes. The lower level causal type in this case is the concatenation of the nodal types for $E$, nodal types for $I$ and the nodaly type for $D$, which specified how $D$ responds to $E$ and $I$. Thus we can think of the the higher level type as depending uniquely on the fully specified lower level type.

For instance, a case has type $\theta_{01}$ in the higher-level model if it has type $\theta_{01}^{11}$ in the lower-level model _and_ $E=0$. This is a case for which $I$ has a positive effect on $D$ when $E=0$ _and_ in which $E$ _is in fact_ 0. On the other hand, the same lower level type, $\theta_{01}^{11}$, in combination with $E=1$ maps onto the type $\theta_{11}$ in the higher-level model---a type in which $D$ is 1 regardless of $I$'s value. 

Meanwhile, the lower-level type $\theta_{10}^{10}$ maps onto the higher-level type $\theta_{10}$ regardless of the data ($E$'s value) since this lower-level type implies that $I$ has a negative effect on $D$ under any value of $E$. 
...
We clsoe this chapter by considering how the understanding of theory that we work with in this book compares to other prominent understandings of theory. 
