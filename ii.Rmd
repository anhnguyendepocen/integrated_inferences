...
---------------------------------------------------------------------------------------------
$\theta^Y$          if $X_1=0, X_2=0$   if $X_1=0,X_2=1$  if $X_1=1,X_2=0$   if $X_1=1, X_2=1$ 
-----------------   -----------------   ----------------  -----------------  -----------------
...
| subscript position |  Interpretation                         |
|:------------------:|:---------------------------------------:|
| 1                  | Value of D when:  I = 0 & P = 0 & M = 0 |
| 2                  | Value of D when:  I = 1 & P = 0 & M = 0 |
| 3                  | Value of D when:  I = 0 & P = 1 & M = 0 |
| 4                  | Value of D when:  I = 1 & P = 1 & M = 0 |
| 5                  | Value of D when:  I = 0 & P = 0 & M = 1 |
| 6                  | Value of D when:  I = 1 & P = 0 & M = 1 |
| 7                  | Value of D when:  I = 0 & P = 1 & M = 1 |
| 8                  | Value of D when:  I = 1 & P = 1 & M = 1 |
Table: (\#tab:interpretation) Interpreting the eight subscripts on $\theta^D$ given 3 parents.


Thus, to illustrate, $\theta^D_{01011010}$ represents a type in which inequality always (regardless of pressure) has a positive direct effect on democratization when $M=0$ and always has a negative direct effect on democratization when $M=1$; in which pressure itself never has an effect; and in which mobilization  has a positive effect when inequality is low but a negative effect when inequality is high.
...
For a case in which only partial data are observed, we follow the same basic logic as with partial process-tracing data. We retain all columns (data types) in the ambiguity matrix that are consistent with the partial data. So, for instance, if we observe only $Y=1$, we woudl retain both the $X=0, Y=1$ column and the $X=1, Y=1$ column. We then calculate the event probability by summing causal-type probabilities for all causal types that could have produced these partial data --- i.e., all those with a $1$ in *either* column.
...
Thus, assuming that each observed case represents an independent, random draw from the population, we can form the likelihood function as a product of multinomial distributions. For instance, for a model with 
...
We provide some tools for both of these  approaches and illustrate them below for hte running example and the democracy application.
...
kable(
  query_model(model, queries = list(ATE = "Y[X=1] - Y[X=0]"), 
              using = rep(c("parameters", "priors"), each = 3), 
...
# Calculate the probabiity of each data type given the reference model.  
  ...
  write_rds(diagnosis_n1, "saved/diagnosis_n1.rds")
...
```{r showx1y1diagnosestrategies, eval=FALSE}
diagnosis_X1Y1 <- 
  diagnose_strategies(
    reference_model = reference_model, 
    analysis_model  = model,
...
model <- make_model("X -> Y <- Z -> K") %>%
          ...
          set_parameters(type = 'define',
              alphas = list(
                       Y = c(`(Y[X=1, Z = 1] > Y[X=0, Z = 1])` = 10), 
                       K = c(`(K[Z = 1] > K[Z = 0])` = 5)
                       ))
```
...
In providing a structure for these tasks we also gain insight into problems with how we were thinking of these things before... Includes that priors and probative value can't be independent.
