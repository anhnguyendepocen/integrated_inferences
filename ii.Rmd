...
In Chapter \@ref(questions), we described estimands of interest as queries over nodal types in causal models. 

Once queries are defined in terms of nodal types, the formation of beliefs, given data $W$, about estimands follows immediately from application of Bayes rule. 
...
$$P(q | W) = \int_{\theta:Q(\theta) = q} P(\theta|W)du =  \int_{\theta:Q(\theta) = q} \frac{P(W|\theta)P(\theta)}{\int_{\theta'}P(W|\theta')P(\theta')du'}du$$
