...
list(df = df, observed = observed) %>%
  write_rds("saved/ch_13_walkkthrough.rds")
...
ch_13_walkkthrough <- read_rds("saved/ch_13_walkkthrough.rds")
...
#    average_data_probabilities(reference_model, s, using = "posteriors", sims = 3000)
     ...
     # FLAG -- WOULD BE FASTER TO HAVE THIS
     # RUN FROM TRANSFORMED_PARAMETERS INSTEAD OF PARAMETERS
...
```{r chselillustration, echo = FALSE, warning = FALSE}  
ch_13_walkkthrough$df %>%
  ...
  kabble(caption = "Each column shows a possible distribution of data that can be generated from a given strategy. We calculate the probability of each data possibility, given the data seen so far, and the posterior variance associated with each one.")   %>%  kable_styling(latex_options = "scale_down")
```
...
write_rds(test_corr, "saved/13_test_corr.rds")
...
write_rds(test_flat, "saved/13_test_flat.rds") 
}

test_corr <- read_rds("saved/13_test_corr.rds") 
test_flat <- read_rds("saved/13_test_flat.rds") 
...
if(do_diagnosis){
  ...
  write_rds(updated, "saved/13_longupdated.rds") 
}
updated <- read_rds("saved/13_longupdated.rds") 
