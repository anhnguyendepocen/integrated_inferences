...
model <- make_model("X->M->Y")  %>%
 ...
 strategies <- 
   list(A_on_regression_line, B_off_regression_line, C_X1Y1_only)
 	...
 	write_rds(strategies, "saved/13_illustration_strategies.rds")
 ...
 estimates_dbs <- lapply(strategies, function(s) 
	...
	write_rds(estimates_dbs, "saved/13_illustration_estimates.rds")
...
#    average_data_probabilities(reference_model, s, using = "posteriors", sims = 3000)
   ...
   apply(reference_model$posterior_distribution, 1, function(pars) {
	...
	write_rds(probabilities, "saved/13_illustration_probabilities.rds")
...
kabble(df, caption = "Each column shows a possible distribution of data that can be generated from a given strategy. We calculate the probability of each data possibility, given the data seen so far, and the posterior variance associated with each one.", digits = 2)   %>%  kable_styling(latex_options = "scale_down")
```
...
kabble(data.frame(Strategy = c("Online", "Offline", "X=1, Y=1"), Variance = expected_posterior_var), digits = 3, caption = "Expected posterio variances", booktabs = TRUE)
```
