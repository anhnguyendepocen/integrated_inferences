...
Suppose that our query is whether $X$ has a positive effect on $Y$. Using this model we can ask how likely different data realizations are and what we would infer about our query from each possible data realization, given existing data. We illustrate for a situation in which we already know that $X=0$ and $Y=0$. 
...
In the sixth column, we see the inference we would make from each data-realization, the posterior probability that $X$ has a positive effect on $Y$. The final column indicates the probability of each data-realization, given the chosen strategy. FLAG: NEEDS COMPLETING.
...
We operationalize higher levels of expected learning from a strategy as a greater expected reduction in posterior variance. We can see that the clue strategies fall into two groups: one with a greater and one with a lesser expected reduction in uncertainty. All strategies that involve looking for $S$ have the same expected posterior variance, which is lower than the expected posterior variance of all strategies that exclude a search for $S$. The implication that if we know $X=0$ and $Y=0$ and we are interested in finding out whether $X=0$ *because* $Y$ is 0, then we should look for evidence on $S$. Moreover, *no other information is useful at all* as we get no further reduction in expected posterior variance. Thus, if the search for information is costly, the dominant strategy is the first, looking only for $S$.
...
We now apply this approach to the model of democratization that we worked with Chapters \ref(ptapp) and \ref(mixingapp).
