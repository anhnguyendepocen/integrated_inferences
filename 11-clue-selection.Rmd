...
```{r, echo = FALSE}
strategy_list_pimd_confound2 <- strats(pimd_confound2, c("P", "M"))

# Assume prices in wihch X and C are somewhat expensive
prices_pimd_confound2 <- c(1, .5, 1.2, .5)

# do_diagnosis <- TRUE
if(do_diagnosis){
results_pimd_confound2 <- t(apply(strategy_list_pimd_confound2, 1, function(j) 
  strategy_evaluation(pimd_confound2, 
                      strategy  = j, 
                      given = "D==1 & I==1", 
                      query = "(D[I=1] != D[I=0])", 
                      prices = prices_pimd_confound2))) %>%
  data.frame()
  write_rds(results_pimd_confound2, "saved/ch11_tradeoffs_pimd2.rds")
  }
results_pimd_confound2 <- read_rds("saved/ch11_tradeoffs_pimd2.rds")

kable(data.frame(cbind(strategy_list_pimd_confound2, results_pimd_confound2) ), digits = 4)
# Plot results
with(results_pimd_confound2, plot(expected_costs, expected_variance, main = "Tradeoffs over strategies | Democracy model with confound"), xlab = "Expected costs of strategy", ylab = "Expected posterior variance from strategy")
