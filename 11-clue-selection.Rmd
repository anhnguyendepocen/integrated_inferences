...
To isolate how the query matters from how the restrictions matter, we first diagnose clue strategies for a causal-effect query under this new model: given $I=0, D=1$, did $I$ have a negative effect on $D$? Inferences conditional on data-realizations and clue strategies are displayed in Table \@(possible_outcomespimd_priors_effect) and expected posterior variances in Table \@ref(pimdlearn_priors_effect). 
...
Turning to expected learning from alternative strategies, shown in Table \@ref(pimdlearn_path), we can see clearly -- by comparison to Table \ref@(pimdlearn_priors_effect) -- how optimal clue strategies depend on the query of interest. Whereas $M$ is only slight informative about $I$'s causal effect--with $P$ the more informative clue--we expect to learn much more from $M$ about the pathway-specific query, and $M$ is far more informative than $P$.
...
# Assume prices in which X and C are somewhat expensive
prices_pimd <- c(1, 1.5, 1, 1)
...
resultsfile_pimd <- read_rds("saved/ch11_tradeoffs_pimd.rds")

kable(data.frame(cbind(strategy_list_pimd, resultsfile_pimd) ), digits = 4)
# Plot results
with(resultsfile_pimd, plot(expected_costs, expected_variance, main = "Tradeoffs over strategies | Democracy model"), xlab = "Expected costs of strategy", ylab = "Expected posterior variance from strategy")
...
FLAG: Have the table and graph display separately, each with a label. Insert table label into text.
FLAG: Label the 4 non-dominated strategies.
...
```{r, echo = FALSE}
strategy_list_pimdconfound2 <- strats(pimd_confound2, c("P", "M"))

# Assume prices in wihch P is more expensive than M
prices_pimdconfound2 <- c(1, 1.5, 1, 1)

# do_diagnosis <- TRUE
if(do_diagnosis){
results_pimdconfound2 <- t(apply(strategy_list_pimdconfound2, 1, function(j) 
  strategy_evaluation(pimd_confound2, 
                      strategy  = j, 
                      given = "D==1 & I==1", 
                      query = "(D[I=1] != D[I=0])", 
                      prices = prices_pimdconfound2))) %>%
  data.frame()
  write_rds(results_pimdconfound2, "saved/ch11_pimd2.rds")
  }

resultsfile_pimdconfound2 <- read_rds("saved/ch11_pimd2.rds")

kable(data.frame(cbind(strategy_list_pimdconfound2, resultsfile_pimdconfound2)), digits = 4)
# Plot results
with(resultsfile_pimdconfound2, plot(expected_costs, expected_variance, main = "Tradeoffs over strategies | Democracy model with confound"), xlab = "Expected costs of strategy", ylab = "Expected posterior variance from strategy")
