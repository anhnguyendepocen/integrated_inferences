...
The proposed approach is one in which researchers build causal models, update those models using data, and then query the models to get answers to particular causal questions. This is very  different to an approach in which researchers focus on selecting the best estimator to estimate a particular estimand of interest. In this approach the model itself gets updated, not the estimate, the focus is on processes, not individual effects.

We do not claim that this approach is the best or only approach to addressing causal questions. And we can readily come up with settings where other approaches work better. But we do think that the approach holds considerable promise, allows researchers to combine disparate data in a principled way to ask a variety of causal questions, and should have a place in the applied researcher's toolkit. 
...
To caricature positions a little bit we might imagine approaches favored by an engineer and a skeptic. The engineer  tackles the problem of causal inference using models: theories of how the world works, generated from past experiences and applied to the situation at hand. The skeptic maintains a critical position, resisting models and the importation of beliefs not supported by evidence in the case at hand. 
