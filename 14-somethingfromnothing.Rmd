...
In the process tracing applications we saw no updating on the causal models---updating was only with respect to case level causal types. 
...
Imagine a fortunate situation in which (a) it is known that the true causal model has the form $X \rightarrow M \rightarrow Y$ and (b) we have a lot of experimental data on the conditional distribution of $M$ given $X$ and of $Y$ given $M$. 

Let us define:

* $\tau_1 = \Pr(M=1 | X=1) - \Pr(M=1 | X=0)$
* $\rho_1 = \Pr(M=1 | X=1) - \Pr(M=0 | X=0)$
* $\tau_2 = \Pr(Y=1 | X=1) - \Pr(Y=1 | M=0)$
* $\rho_2 = \Pr(Y=1 | M=1) - \Pr(Y=0 | M=0)$

These are all quantities taht can be calculated from teh data. The $\tau$s are average treatment effects and the $\rho$s are indicators for how common the $Y=1$ outcome is.
...
which we can see is decreasing in $\lambda_{b_j}$ (this may seem counterintuitive, but the reason is that with $\tau_j$ fixed, lower $\lambda_{b_j}$ also means lower $\lambda_{a_j}$ which means less ambiguity about *how* $X$ affects $Y$ (ie through positive or negative effects on $K$).
...
```{r probval1, echo = FALSE, fig.width = 8, fig.cap = "Bounds on probative value"}
