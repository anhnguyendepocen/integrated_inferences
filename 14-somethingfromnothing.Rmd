...
The grounded approach we described provides an answer to this puzzle. In short, knowledge of the structure of a causal model, together with data on exchageable units, can be enough to place bounds on possible values of $\phi_{b}, \phi_{d}$. 
...
WE first consider a situation in which we believe the same model holds in multiple sites but learnig about the model requires combining data about different parts of the model from different sutes. 
...
df[1:(N/3), "K"] <- NA
df[(1+N/3):(2*N/3), c("X", "Y")] <- NA
df[(1+2*N/3):N, "Z"] <- NA

updated1 <- gbiqq(model, df[(1+2*N/3):N, ], stan_model = fit)
updated2 <- gbiqq(model, df, stan_model = fit)

# If updating done using case data only
result1 <- query_model(updated1, 
            queries = "Y[X=0] == 0", 
            subsets = list(
              "X == 1 & Y == 1 & K == 1",
              "X == 1 & Y == 1 & K == 0"), 
...
result2 <- query_model(updated2, 
            queries = "Y[X=0] == 0", 
            subsets = list(
              "X == 1 & Y == 1 & K == 1",
              "X == 1 & Y == 1 & K == 0"), 
            using = "posteriors")
write_rds(list(result1, result2, updated1, updated2), "saved/frankenstein.rds")
}
...
kable(frank[[1]][,-c(1,3)], caption = "Learning from single site does not yield and informative clue")

kable(frank[[2]][,-c(1,3)], caption = "Clue is informative after combining studies linking $K$ to $Z$ and $Z$ to $Y$")

```


Consider next a problem where the models partially differ across sites: for instance we believe that although treatment effects are different in two sites yet the mechanisms linking treatment to outcomes are the. As a simple example we might imagine that $X$ is differentially likely to produce $M$ in two sites, but if it does the relation between $M$ and $Y$ is common across sites.
