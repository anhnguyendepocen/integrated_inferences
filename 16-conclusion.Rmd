...
The challenge is one of fragmentation. Not only do I need access to your evidence, but we have to be operating to some degree with a common causal model of the domain of interest; else, the data that you generate might simply fail to map onto my model. Moreover, if we are all updating our own, separate models, the prospects for agreement about what "we" have learned are likely to be dim. And there is little reason to be optimistic that individual researchers will naturally tend to generate models that align with one another. Not only might our substantive causal beliefs diverge, but we are likely to make differing choices about matters of model-construction --- from which nodes to include and the appropriate level of detail to the manner of operationalizing variables. 

Turning causal models into vehicles for knowledge cumulation in a field will thus require coordination.
