...
Another possibility would be to allow for modularity, with researchers modeling different *parts* of a causal system.

Another possibility, involving less coordination, would be to allow researchers to develop models that agree only on the inclusion of a minimal set of outcome nodes. As new data comes in, models would then be set in competition over predictive performance.

Coordination would also require agreement on some minimal set of qualities that included models must have. For instance, we would surely want all models to be well defined, following the basic rules of DAG-construction and with clear specification of all nodes. 

If we want new data to allow for updating across multiple models, then those models must have at least some shared nodes. 

Given that researchers will usually be studying a phenomenon across contexts, 
