...
We would want models to be grounded, in some principled way, on information from outside the model. There are number of potential ways in which models might be justified. One kind of grounding is theoretical. As we show in the appendix to Chapter \@ref(theory), for instance, the implications of a game-theoretic model can be readily translated into a causal model. Clear informal theoretical logics could underwrite causal models as well. A second form of grounding would be empirical. As we discuss in Chapter \@ref(justifying), experimental data can be used to anchor model assumptions. A third approach would be subjective-Bayesian: models could be founded in aggregated expert beliefs. For instance, it would not be hard to imagine researchers gathering and aggregating prior causal beliefs from a set of experts within a field of inquiry.
