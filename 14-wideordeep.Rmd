...
We can also use these results to think through optimal allocations of resources with varying prices of breadth and depth. In the unconstrained chain model, we see similar posterior variance, when estimating the probability of positive causation, from the following three combinations of wide and deep: 

1. Maximally wide: 1600 wide + 0 deep
2. Maximally deep: 100 wide + 100 deep
3. Mixed: 400 wide + 50 deep

However, which strategy is optimal will depend on the relative cost of collecting $X,Y$ data for a new case (let this be $w$) and collecting $M$ for an existing case ($d$).

Assume, to simplify things, that $w=1$. Then, for this model-query combination, the widest strategy is better than the deepest strategy if and only if $1600 < 100 + 100d$, that is, when $d > 14.3$. The mixed strategy is better than the maximally deep strategy if and only if $400 + 50d < 100 + 100d$, that is when $d > 5.9$. And the maximally wide strategy is better than the mixed strategy if and only if $1600 < 400 + 50d$, or $d > 25$. Thus, roughly speaking, if $d > 25$, our preference-ordering is widest > mixed > deepest. If $14.3 < d < 25$, our ordering is mixed > widest > deepest. And if $5.9 < d < 14.3$, our ordering is mixed > deepest > widest. If $d < 5.9$, then our ordering is deepest > mixed > widest. We can, thus, see that the mixed strategy is optimal across a broad range of $d$, anywhere from about $5.9$ to $25$.
