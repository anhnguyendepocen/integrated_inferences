...
Structural models can imply other simpler structural models. Take the very simple model, $M$, represented in Figure \@ref(fig:Highlow)(a). The model simply states that $X$ has (or *can* have) a causal effect on $Y$. 
...
A key idea, which we return to later, is that both $M'$ and $M''$ involve a redefinition of $\theta^Y$. That is we see a change in the endogenous nodes but these in turn imply a change in the interpretation of the exogenous nodes pointing into existing endogenous nodes (such as $Y$ in this example). We return to the quantitative implications of this redefinition of endogeneous nodes below. 
...
<!-- We have alluded to the fact that we may want to theorize our models --- develop more elaborate, lower-level models to support them --- because we can get potentially reap greater inferential leverage from the more elaborate theory. But how much more? For instance, what are the gains from a theory that introduces a node $K$ relative to one that does not include $K$? What is the value-added of the more elaborate theory? -->
