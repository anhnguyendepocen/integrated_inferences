...
kabble(possible_inferences[possible_inferences$prob >0, ], caption="Inferences given different data patterns, with observations on some or all nodes.", row.names = FALSE, digits = 3)
```
...
kabble(dfl, col.names = c("Strategy", "Given", "Prior belief", "Prior Uncertainty", "Expected Posterior Uncertainty"), caption = "Expected learning from different clue selection strategies. Each strategy is a group of nodes on which within case data is gathered.")  %>%  kable_styling(latex_options = "scale_down")
```
...
if(do_diagnosis)
  conditional_inferences(model, 
                         query = list(COE = "(Y[X=1] > Y[X=0])"), 
                         given = "Y==0") %>%
  write_rds("saved/12_condinf2.rds")

possible_inferences <- read_rds("saved/12_condinf2.rds")
```
...
if(do_diagnosis)
  ...
  write_rds("saved/12_learning2.rds")

learning <- read_rds("saved/12_learning2.rds")
```
...
dfl <- t(learning) %>% data.frame() %>%
  mutate(prior_estimand= round(as.numeric(prior_estimand), 3),
         prior_var = round(as.numeric(prior_var), 3),
         E_post_var = round(as.numeric(E_post_var), 3))
kabble(dfl, col.names = c("Strategy", "Given", "Prior belief", "Prior Uncertainty", "Expected Posterior Uncertainty"), caption = "Expected posterior variance from different clue selection strategies (less restricted model).")  %>%  kable_styling(latex_options = "scale_down")
...
kabble(dfl, col.names = c("Strategy", "Given", "Prior belief", "Prior Uncertainty", "Expected Posterior Uncertainty"), caption = "Expected posterior variance from different clue selection strategies. (Stonger priors on S=1)")  %>%  kable_styling(latex_options = "scale_down")
```
...
kabble(inferences_pimd, caption = "Table shows possible data patterns for P and M given I = 1 and D = 1 together with the probability of observing each data realization given data is sought on a variable and the posterior on the query (does $I$ have a positive effect on $D$) given that data realization.", digits = 3, row.names = FALSE)
...
write_rds(ELpimd1, "saved/12_ELpimd1.rds")
}

ELpimd1 <- read_rds("saved/12_ELpimd1.rds")

x_strategy <- c("None", "P", "M", "P and M")

kabble(cbind(strategy = x_strategy, ELpimd1[,-1]), caption = " Prior estimand, prior variances and expected posterior variances for the query (does $I$ have a positive effect on $D$?) given different clue seeking  stratgies for cases in which we have observed high inequality and democratization.", digits = 3)
...
kabble(inferences2_pimd, caption = "Table shows possible data patterns for P and M given I = 0 and D = 1 together with the probability of observing each data realization given data is sought on a variable and the posterior on the query (does $I$ have a negative effect on $D$) given that data realization.", digits = 3, row.names = FALSE)
...
kabble(cbind(strategy = x_strategy, ELpimd2[,-1]), caption = "Prior estimand, prior variances and expected posterior variances for the query (does $I$ have a negative effect on $D$?) given different  clue seeking  stratgies for cases in which we have observed low inequality and democratization.", digits = 3, row.names = FALSE)
...
<!-- kabble(inferences_pimdcon, caption = "\\label{possible_outcomespimd_i1con} Table shows possible data patterns for P and M given I = 1 and D = 1, and assuming confounding, together with the probability of observing each data realization given data is sought on a variable and the posterioron the query (does $I$ have a positive effect on $D$) given that data realization.", digits = 3) -->
...
kabble(inferences_pimdconf, caption = "Table shows possible data patterns for P and M given I = 1 and D = 1 together with the probability of observing each data realization given data is sought on a variable and the posterior on the query (does $I$ have a positive effect on $D$) given that data realization, with confounding involving $P$.", digits = 4, row.names = FALSE)
...
kabble(inferences_priors_effect, caption = "Table shows possible data patterns for P and M given I = 0 and D = 1 together with the probability of observing each data realization given data is sought on a variable and the posterior given that data realization for the query: does I have a negative effect on D?", digits = 3, row.names = FALSE)
...
kabble(inferences_pimdpath, caption = "Table shows possible data patterns for P and M given I = 0 and D = 1 together with the probability of observing each data realization given data is sought on a variable and the posterior given that data realization for the pathway query: does I have a negative effect on D that is mediated by M?", digits = 3, row.names = FALSE)
...
if(do_diagnosis)
  rbind(
    ...
    expected_learning(pimd_priors, query = "(M[I=1] < M[I=0]) & 
                     (D[M=M[I=0]] > D[M=M[I=1]]) & 
                     (D[I=1, M=M[I=1]] == D[I=0, M=M[I=1]])", strategy = c("P","M"), given = "I==0 & D==1")) %>% write_rds("saved/12_ELpimd_path.rds")

# write_rds(ELpimd_path, "saved/ch12_ELpimd_path.rds")

ELpimd_path <- read_rds("saved/12_ELpimd_path.rds")

x_strategy <- c("None", "P", "M", "P and M")

kabble(cbind(strategy = x_strategy, ELpimd_path[,-1]), caption = "Prior estimand, prior variances and expected posterior variances for the query (does I have a negative effect on D that is mediated by M?) given different  clue-seeking  stratgies for cases in which we have observed low inequality and democratization.", digits = 4)
```
...
results_pimd_tradeoff_confound <-
  strategy_evaluation(
    ...
    write_rds(results_pimd_tradeoff_confound, "saved/12_tradeoffs_pimd_confound.rds")
  }

results_pimd_tradeoff_confound <- read_rds("saved/12_tradeoffs_pimd_confound.rds")
