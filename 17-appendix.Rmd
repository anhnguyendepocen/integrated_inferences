...
This is the DAG in figure 4C of @bareinboim2016causal. The key feature is that data is only seen for units with $S=1$.
...
fusion <- set_restrictions(fusion, 
                           c("(S[W1=1] < S[W1=0])",
                             "(W2[W1=1] < W2[W1=0])", 
                             "(X[W2=1] < X[W2=0])", 
                             "(Y[Z=1] < Y[Z=0])")) %>%
          set_priors()   %>%
          set_parameters(type = "prior_draw")
            
plot_dag(fusion)

query_model(fusion, queries = "Y[X=1] - Y[X=0]", subsets = c(TRUE, "S==1"))

df1 <- simulate_data(fusion, n = 10) %>% filter(S==1)
df2 <- mutate(df1, Z = NA)
df3 <- mutate(df1, W1 = NA, W2 = NA)
