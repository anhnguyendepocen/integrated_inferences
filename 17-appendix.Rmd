...
We sometimes might wonder whether qualitative information is bound to get drowned out by quantiative information. But in fact sometimes a lot of quantitiatve information can render qualitiative information highly informative. 
...
XY_data <- data_strategy(model, n_obs = 10000, 
                         vars = list(c("X", "Y")), 
                         probs = 1, using = "parameters")

mixed_data <- data_strategy(model, 
                            n_obs = 10000, 
                            vars = list(c("X", "Y"), c("M1", "M2")),
                            probs = list(1, .01), 
                            subsets = list(NULL, "X==1"))
  
if(!exists("fit")) fit <- gbiqq::fitted_model()

updated_XY    <- gbiqq(model, XY_data,    stan_model = fit)
updated_mixed <- gbiqq(model, mixed_data, stan_model = fit)
...
query_model(updated_XY, 
  queries = list(ate = te("X", "Y"), X_on_M1 = te("X", "M1"), X_on_M2 = te("X", "M2")), 
