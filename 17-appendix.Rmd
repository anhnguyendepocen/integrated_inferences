...
model <- make_model("X -> M -> Y") %>%
         ...
         set_parameters(node = "X", confound = "M[X=1]!=1", alphas = c(.2, .8)) %>% 
...
selection <- set_parameters(selection, {
...
selection
df  <- simulate_data(selection, n = 60000, using = "parameters")
df0 <- filter(df, S==1)
...
lm2 <- lm(Y~X*Z_norm, data = mutate(selection_list$df0, Z_norm = Z- mean(Z)))
