...
The simplest $X$ cuases $Y$ model without confounding  can be written:
...
With this  sparse definition we can construct a parameter matrix that maps from parameters to causal types. Inspecting the parameter matrix,  confirms assumes that there are no constraints here on possible nodal types  and no unobserved confounding.
...
We will  assume a "true" distribution over parameters. In the distribution belwo we will assume there is a true positive effect of 0.5.

```{r}
model <- set_parameters(model, c(.5, .5, .2, .1, .6, .1))

kable(t(model$parameters))
```
...
From the updated model we can draw posterior inferences over estimands of interest. We will imagine three estimands, corresponding to Pearl's "ladder of causation." At the first level we are intersted in the distribution of some node, perhaps given the value of another node.  At the second  level we are interested in treatment effects: how changing one node changes another. THis question is answerable from an experiment. At the third level we are interested in counterfactual statements: how would things have been different if some features of the world were different from what they are?
...
We see from the posterior variance on PC that we have the greatest difficulty on the third rung. In particular the PC is not identified (the  distribution does not tighten even with very large N).  For more intuition we graph the posteriors:
...
## An $X$ causes $Y$, with confounding modelled
...
```{r, eval = FALSE}
fusion <- make_model("X <- W1 -> W2 -> X -> Y <- Z -> W2; W1 -> S")
```
...
fusion <- make_model("X <- W1 -> W2 -> X -> Y <- Z -> W2; W1 -> S",
    ...
    set_restrictions(c(
                     ...
                     "(Y[X=1] < Y[X=0])",
                     "(Y[X=1, Z=0] > Y[X=0, Z=0])")) %>%
    set_restrictions(labels = list(W2 = "W20001"), keep = TRUE) %>%
    set_priors() %>%
    set_parameters(type = "flat")

#Manual hack for 0 effect  

fusion <- set_parameters(fusion, {
    parameters <- fusion$parameters
    parameters["W1.W10"] <- 2/3
    parameters["W1.W11"] <- 1/3
    parameters["S.S01"]  <- 2/3
    parameters["Y.Y0001"] <- 0
    parameters}
    )

fusion
df  <- simulate_data(fusion, n = 60000, using = "parameters")
df0 <- filter(df, S==1)
df1 <- mutate(df0, Z = NA, W1 = NA, W2 = NA)
df2 <- mutate(df0, Z = NA)
df3 <- mutate(df0, W1 = NA, W2 = NA)


# mean(df$W1)
# mean(df0$W1)
# summary(lm(X~Z, data = df0))
# summary(lm(X~Z, data = filter(df0, W1==1, W2==1)) # But not conditional on W1, W2
...
M1 <- gbiqq(fusion, df1, stan_model = fit)
M2 <- gbiqq(fusion, df2, stan_model = fit)
M3 <- gbiqq(fusion, df3, stan_model = fit)

OUT0 <- query_model(fusion, 
                    using = c("parameters", "priors"), 
                    queries = "Y[X=1] - Y[X=0]")
OUT <- lapply(list(M1, M2, M3), function(M) 
  query_model(M, using = "posteriors", queries = "Y[X=1] - Y[X=0]"))

write_rds(list(M1=M1, M2=M2, M3=M3, OUT0=OUT0, OUT=OUT, df0=df0, df1=df1, df2=df2, df3=df3, fusion), "saved/17_fusion_list.rds")
}

fusion_list <- read_rds("saved/17_fusion_list.rds")
...
kable(fusion_list$OUT0, caption = "Estimand values")
```
...
lm1 <- lm(Y~X,  data = fusion_list$df0)
lm2 <- lm(Y~X*Z_norm, data = mutate(fusion_list$df0, Z_norm = Z- mean(Z)))
stargazer(lm1, lm2, type = 'html', header = FALSE)
```
...
kable(
  ...
  bind_rows(fusion_list$OUT)[, c("mean", "sd")]
  # [c(1,3,5),]
  ))
...
Note: this does not reproduce the problem that should arise from conditioning on W1 and W2.
