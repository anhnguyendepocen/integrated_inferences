...
list(`1` = q1, `2` = q2) %>% bind_rows(.id = "Model") %>% filter(Using == "posteriors") %>% 
  select(Model, Given, mean, sd) %>% arrange(Given) %>% kabble(caption = "Can observation of large N data render mediator $M$ informative for case level inference? Model 1: No knowledge of structure; Model 2: $X$ known to be randomized. Posterior SD in parentheses.", row.names = FALSE)
