...
p1 <- hj_ggdag(model=model_1, x = c(0, .5, 1), y = c(.5,1,0))
...
```{r ch15nino, echo = FALSE, fig.cap = "Can observation of large N data render mediator $M$ informative for case level inference? Model 1: No knowledge of structure; Model 2: $X$ known to be randomized. Error bars show two posterior standard deviations."}
...
list(`1: No assumptions` = q1, `2: X randomized` = q2) %>% bind_rows(.id = "Model") %>% filter(Using == "posteriors") %>% ggplot(aes(Given, mean, color = Model)) + geom_point(position = pd) +
  geom_errorbar(aes(ymin = mean - 2*sd, ymax = mean + 2*sd, width = .2), position = pd) + coord_flip()
...
This example nicely illustrates the @cartwright1994nature's idea of "no causes in, no causes out." It also poses, we think, a challenge to any process-tracing exercise that aspires to  model-independence: observational data alone is not sufficient to generate a justification for process-tracing inferences for 3-node problems *even when in reality* causal structures are simple.
...
tests <- lapply(tests, function(d) select(d, - Case.estimand))
...
for(j in 4:7){
  teststable[,j] <- round(teststable[,j], 3)
