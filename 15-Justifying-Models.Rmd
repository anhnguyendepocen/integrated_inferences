...
write_rds(agnostic_XMY, "saved/ch14_agnostic.rds")
...
write_rds(agnostic_query, "saved/ch14_agnostic_query.rds")


}

agnostic_query <- read_rds("saved/ch14_agnostic_query.rds")
...
kabble(table, col.names = c("", "Doubly decisive", "Hoop", "Smoking gun", "Straw in the wind"),
             caption = "Known probabilities from a model with  $X \\rightarrow Y \\leftarrow K$  justifying classic test types for clue $K$ given $X=Y=1$.", align=c("l", rep('c', 4)))   %>%  kable_styling(latex_options = "scale_down")
...
```{r disoveryplotsa, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "DAGs from three structural causal models.", fig.width = 8, fig.height = 5, results='hide',fig.keep='all', eval = TRUE, fig.align = "center", out.width = "80%"}
#par(mar = rep(3, 4))
