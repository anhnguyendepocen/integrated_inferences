...
We will sometimes refer to the values of $\Theta$ as a unit's *context*. This is because $\Theta$ contains the assignment process for the model's exogenous nodes as well all contextual factors that shape causal relations between nodes. Put differently, $\Theta$ captures both how a unit reacts to situations (the $\theta^J$'s for endogenous nodes) but also which situations it is reacting to (the $\theta^J$'s for exogenous nodes). There is thus no *formal* distinction between a unit's type and a unit's situation---between, say, a hungry person, and a person who has had no food.
...
causal type       |$\Theta$   | A causal type is a concatenation of nodal types, one for each node. Example:  $(\theta^X_0, \theta^Y_{00})$, is a causal type that has $X=0$ and that has $Y=0$ no matter what the value of $X$. 
::::
