...
<!-- AJ: We had "For instance, a structural causal model might support a claim of the form: "$R$ has a positive effect on $M$ if and only if $E=1$ holds." I found this confusing because while, yes, it's a function, it's not the kind of function we move forward with for the rest of the book. No types. I think what I've changed this to will be more helpful for readers. -->

In the default setup, we assume that each $\theta$ term ($\theta^I, \theta^E, \theta^R$, etc.) is generated independently of the others---that each node's type for a given case is drawn from an independent distribution. Whether a given case exhibits a positive effect of   While this is not without loss of generality, it is not as constraining as it might at first appear: any graph in which two exogenous nodes are not independent can be replaced by a graph in which these two terms are listed as endogenous (possibly unobserved) nodes, themselves generated by a third node. Note also that one could envision "incomplete probabilistic causal models" in which researchers claim knowledge regarding distributions over *subsets* of $\theta$. WE CAN ALSO DESCRIBE THEM AS HAVING A JOINT DISTRIBUTION FOR SITUATIONS IN WHICH THEY ARE NOT INDEPENDENT. 
