...
Consistent with our discussion above, we will generally use $\theta$ to denote these  exogeneous variables. The reason for this, as we will show, is that we can think of these nodes as capturing the functional relations between endogeneous variables and as being quantities of direct interest for causal inquiry. We more fully develop this point below.
...
We've been looking at graphs, but we want to take some time to unpack their graphical features. A key benefit of causal models is that they lend themselves to graphical representations and in turn those graphs can help with analysis, if we construct the graph using certain rules. however they also put discipline on graphs. In the next section we discuss implied rules for representing a model in graphical form. THe final section  demonstrates how access to a graph facilitates inferecnces of various forms
