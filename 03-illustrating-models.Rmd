...
These basic worldviews, in turn, affect the cost-benefit calculations they make about intervention strategies, via two mechanisms. First, beliefs affect perceptions of the likely security gains from a transformative intervention strategy. Second, these beliefs affect the kinds of strategic capabilities in which leaders invest, which in turn affects the costliness and likelihood of success of alternative intervention strategies. Calculations about the relative costs and benefits of different strategies then shape the choice between a transformative and non-transformative approach to intervention. 

Yet leaders can often only choose one of these options if they decide to intervene at all. The decision about whether to intervene depends, in turn, on at least two kinds of considerations. A leader is more likely to intervene against a target when the nature of the dispute makes the leader's preferred strategy---given their causal beliefs---appear feasible in this situation; yet leaders may also be pushed to intervene by international or domestic audiences.
...
As illustrated in the Pierson example, in a non-parametric framework, each parametric functional equation represents one nodal type for the relevant $\theta$. For instance, though we spare the reader the complexities of the corresponding subscript notation, there is a single value of $\theta^B$ under which the conditions $C=1, P=1,$ and $T=1$ generate $B=1$, and we get $B=0$ otherwise.  Likewise, there exists a single value of $\theta^I$ under which $B=1, C=1$ and $B=0, C=0$ produce $I=1$, for either value of $A$; and $A$ has a positive effect on $I$ whenever $B \neq C$. To work with this model, we would specify a probability distribution over all possible nodal types for each node on the graph.
...
$$D_t = \mathbb{1}(\theta_{D_t} + (1-D_{t-1})p + D_{t-1}Y_{t-1}q) > 0)$$
...
half of cases would be democracies (regardless of income), half of which would be surviving democracies and half new democracies; of the other half, one quarter would be surviving democracies (surviving *because of* their income), and the other three quarters would be autocracies, one third of which would be "backsliders" because of their poverty. Similar transitions occur in future periods until eventually the wealthy states are all democratic and the  poorer states are half democratic and half autocratic, transitioning back and forth randomly each period.

In this non parametric set up there are no parameters $p$ or $q$ to be estimated. Rather the focus is entirely on the distribution of nodal types. 
