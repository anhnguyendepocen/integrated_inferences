...
::: {.center data-latex=""}
:::
We represent three arguments as causal models with associated graphs.
::::
...
While the argument has many more specific features (e.g., different program-structural factors that matter, various potential strategies of blame-avoidance), its essential components can be captured with a relatively simple causal model. We propose such a model in graphical form in Figure \@ref(fig:pierson). Here, the outcome of retrenchment ($R$) hinges on whether retrenchment makes it onto the agenda ($A$) and on whether blame-avoidance strategies are available to governments ($B$), and on some unspecified random input ($\theta^R$). Retrenchment emerges on the policy agenda as a consequence of both socioeconomic developments ($S$) and the ascendance of ideologically conservative political actors ($C$). Inherited program structures ($P$), meanwhile, determine the availability of blame-avoidance strategies.
...
Let us assume all variables are binary, with each condition either absent or present. 

We might then take a parameteric approach and imagine specific functions connecting parents to children, with perhaps some errir. For instance we can capture quite a lot of Pierson's theoretical logic with the following quite simple functional equations:

* $A=CS$, implying that retrenchment makes it on the agenda if and only if  conservatives are in power *and* socioeconomic pressures are high. 
...
Note we might further imagine the addition of "noise." For instance $R = CS$ is observed with probability $p$ and $R = 1-CS$ with probabilty $1-p$. 

A non-parametric approach could also be used. In this case we would define a distribution over $\theta^A$---which could take one of 16 values; over $\theta^B$ which  could take one of 4 values; and $\theta^R$---which could also take one of 16 values. Then choices on the probability distributions fully reflect the ways these variables relate to each other. Notice also that the "parametric" version can be treated as an instance of the non-parametric approach with probability 0 placed on many possible causal types.

Specifically we have:

* $\theta^A=\theta^A_{0001}$ 
* $\theta^B=\theta^B_{01}$
* $\theta^R \in \{\theta^R_{0000}, \theta^R_{0001}\}$. 

Note finally that full specification of the model also requires a distribution over the $\theta$s.  Those $\theta$s with only one value are essentially deterministic and so have a degenerate distribution (e.g. $\Prob(\theta^A=\theta^A_{0001})(b = 1$). For the others a specification of  $\{\lambda^C, \lambda^S, \lambda^P, \lambda^R\}$ specifies the background probability of different events and the probability with which  $R$ is indeed responsive to its parents.
