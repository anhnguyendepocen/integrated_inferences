...
3. **Specify the query in terms of causal types.** For instance, for the simple $X \rightarrow Y$ model, the query "$Y$ responds positively to $X$" can be thought of as a collection of causal types: Q={Type 1, Type 2}.^[More generally a query might be a function of the distribution of causal types.]
...
If we observe only the partial data, $X=1, Y=1$, then we update to the ambiguity matrix in Table \@ref(tab:ambigmedupdatepartial2). Now both corresponding causal types are included, and we sum their posteriors to get the posterior on the query: $0.0769 + 0.3077 = 0.3846$.
...
### A DAG alone does not get us probative value for a single case 
