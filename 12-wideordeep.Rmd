...
given <- gen_given(12,4,4,12)
model     = make_model("X->M->Y")
N_wide = 1
N_deep = 2
make_possible_data(model = model, given = given, 
                N = c(N_wide, N_deep), withins = c(FALSE, TRUE), conditions = c(TRUE, c("X==1 & Y==1")), 
                vars = c(TRUE, "M"))
...
if(do_diagnosis){
  ...
  for(N in c(2, 4)) { for(m in  1:length(models)) {for(g in 1:length(givens)) {	
    filename <-  paste0("saved/Ch12_", names(models)[m], "_", N, "_", names(givens)[g], ".rds")
...
data_strats <- function(N = 4) {
  if(N%%2 != 0) stop("even Ns only please")
  list(
    all_on = list(N=c(4,1), withins = c(FALSE, TRUE), vars = list(c("X", "Y"), "M")),
    all_off = list(N=c(N,N)/2, withins = FALSE, vars = "M",
                   conditions = list("X==0 & Y==1", "X==1 & Y==0"))
  )
}


testsdata <- gbiqqtools::make_possible_data(model, given = NULL, N = c(4,4), withins = c(FALSE, TRUE), vars = list(c("X", "Y"), "M"))
testsdata
