...
if(do_diagnosis){
 ...
 data <- simulate_data(unrestricted_model, using = "parameters", n = n) 
     write_rds(data, "saved/16_data.rds") 
  
 XMYdata <- simulate_data(restricted_model, using = "parameters", n = n) 
     write_rds(data, "saved/16_XMYdata.rds") 
...
rbind(
  ...
  kabble(digits = 3, caption =  "Regression coefficient on $X$ given $M=0$ and $M=1$")
...
<!-- kabble(df, col.names = "Bayes factors", digits = 2, caption = "Posterior odds: the relative likelihood of one model over another") -->
<!-- ``` -->
...
```{r loolik1, echo = FALSE, fig.cap = "LOO likelihoods for different models", message=FALSE, warning=FALSE}
#Leave one out


if(do_diagnosis){
  
  get_loo_likelihood(data, restricted_model, 1000) %>% write_rds("saved/16_loo_restricted.rds")
    
  get_loo_likelihood(data, unrestricted_model, 1000) %>% write_rds("saved/16_loo_unrestricted.rds")
...
```{r loolik2, echo = FALSE, fig.cap = "LOO likelihoods and data counts"}
...
if(do_diagnosis){
  ...
  get_loo_likelihood(XMYdata, restricted_model, 1000) %>% write_rds("saved/16_loo_restricted_2.rds")
    
  get_loo_likelihood(XMYdata, unrestricted_model, 1000) %>% write_rds("saved/16_loo_unrestricted_2.rds")
...
While it is difficult to see this in Figure \@ref(fig:loostronger), the stronger model still performs better here than the weaker model. The likelihood of the data under the stronger model is now `r  formatC(loo_restricted_2$loo_likelihood, format = "e", digits = 2)`, compared to 
...
```{r ch15ranges, fig.cap = "Possible inferences on `X caused Y` given observation of M=0 and M=1 given different possible models. The highest inference when M=0 is lower than the lowest inference when M=1", echo = FALSE, message=FALSE, warning=FALSE}
...
cbind(adverse2, out2) %>% data.frame() %>%
  kabble(
    ...
    caption = "Inferences on the probability that $X$ caused $Y$ upon seeing $M=0$ or $M=1$ for a range of possible values of $\\lambda^Y_{10}$", format='pandoc')
...
```{r pimdmodelsch15, echo = FALSE, message=FALSE, warning=FALSE}
...
rbind(
 ...
 mutate(Given = c("-", "-", "M = 0", "M = 1")) %>%
  ...
  kabble(digits = 3, caption =  "Regression coefficients to assess conditional independence")
...
```{r loopimdplots1, echo = FALSE, fig.cap = "Leave one out likelihoods for restricted and unrestricted democracy and inequality models",message=FALSE, warning=FALSE}
# Leave one out
...
```{r pimdloocomp, echo = FALSE, fig.cap = "LOO data predictions for three PIMD models"}

list(`restricted model` = loo_pimd,  `unrestricted model` = loo_pimdnegmed,  `fully unrestricted model` =  loo_pimdnonmon ) %>% 
  plot_loo()
...
```{r ch16pimdgraphs, echo = FALSE, fig.cap = "ATE of I on D."}
...
kabble(caption = "Four cases with low inequality and  democratization. Question of interest: Was low inequality a cause of democracy? Table shows posterior beliefs for different data for 4 cases given information on $M$ or $P$. Data from Haggard and Kaufman (2012).",
      digits = 3)   %>%  kable_styling(latex_options = "scale_down")
...
q_11 %>%
  ...
  kabble(caption = "Four cases with high inequality and  democratization. Question of interest: Was high inequality a cause of democratization? Table shows posterior beliefs for different data for 4 cases given information on $M$ or $P$. Data from Haggard and Kaufman (2012).",
      digits = 3)   %>%  kable_styling(latex_options = "scale_down")
