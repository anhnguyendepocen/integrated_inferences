...
In our base model we assume that $M$ is monotonic in $I$. How much do conclusions depend on this restriction?  We answer the question by comparing our conclusion to what we would conclude without this assumption. As we will see, the answer depends on the query examined.

We first show results for population inference from a mixed methods analysis. As seen in Table \@ref(tab:15pimdgraphs), our inferences regarding the overall effect of $I$ on $D$ are not very sensitive to this assumption. Our conditional inferences about the probability of causation are, however. In particular, in cases with $I=0, D=1$ we are more likely to think that democratization was due to *low* inequality given the unrestricted model. When we see that in fact there was no mobilization, our attribution increases in the restricted model but decreases in the unrestricted model. In the fully unrestricted model our inferences are not affected at all by observation of $M=0$. 
