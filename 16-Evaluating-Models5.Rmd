...
We first show results for population inference from a mixed methods analysis. As seen in Table \@ref(tab:15pimdgraphs), our inferences regarding the average effect of $I$ on $D$ are not very sensitive to the monotonicity assumption at $M$. However, they are very sensitive to the monotonicity assumptions at $D$: as we can see, the effect goes from around $-0.25$ to $0$ when we remove the restrictions at $D$.  

Our conditional inferences about the probability of negative causation are less sensitive to these assumptions, however. In particular, in cases with $I=0, D=1$ we are about equally likely to think that democratization was due to low inequality given any of the models. Yet what we learn from $M$ does depend on restrictions. If we look for $M$ and see that there was no mobilization, our attribution of the outcome to $I$ *increases* in the fully restricted model but *decreases* in the partially restricted model. In the fully unrestricted model, $M$ is completely uninformative.

Consider first the fully unrestricted model. In this model, we have no prior knowledge about the direction of effects to or from $M$ and so seeing $M=0$ as opposed to $M=1$ is equally consistent with $I$ causing (via an indirect chain of mixed-direction effects or via the direct pathway) or not causing $D=0$. 

Now let's compare the partially and fully restricted models. In the fully restricted model, $M$ cannot serve as part of a pathway for $I$'s negative effect since any effects along the indirect path must be positive. In this model, $M$ can only matter as a possible rival cause of democratization. If we observe $M=1$, we know that (a) $M$'s value could not have been caused by $I$ and (b) $M$ is a possible rival cause of $D=1$. Thus, our belief that $I=0$ is a cause of $D=1$ diminishes. On the other hand, if we observe $M=0$, our reasoning runs in the opposite direction: we have learned about the absence of a rival cause of $D=1$, making it more likely that $I=0$ is the cause. 

By contrast, in the partially restricted model, $M$ is potentially both a pathway for a negative effect (since negative $I \rightarrow M$ effects are permitted) and a rival cause. Now, seeing $M=1$ tells us that an indirect negative effect might be operating while $M=0$ rules this out. While $M=0$ rules out a rival cause of $D=1$, on balance ruling out the indirect path weighs more heavily in our assessment of $I$'s effect on $D$. Thus, $M=0$ reduces our estimate of $I$'s negative effect in this model, while it increased our estimate in the fully restricted model.
