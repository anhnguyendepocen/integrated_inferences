...
read_rds("saved/11_fusionestimands.rds") %>% kabble(caption = "Estimands in different sites", digits = 2)
...
result <- query_model(
    ...
    given = list(TRUE, "R==0", "R==1"),
    using = "posteriors")
kabble(result, caption = "Estimates on the ATE for observational ($R=0$) and experimental ($R=1$) set.")
```
...
kabble(result2, caption = "Effects of $X$ conditional on $X$ for units that were randomly assigned or not.  Effects of $X$ do not depend on $X$ in the experimental group, but they do in the observational group becuase of seld selection. ")
```
...
read_rds("saved/11_appev2.rds")  %>% 
  kabble(caption = "Priors and true values (parameters) for three estimand: the frequency of $W$, the effect of $X$ on $Y$, and the effect conditional on $W=1$")
...
kabble(q, caption = "Extrapolation when two sites differ on $W$ and $W$ is observable in both contexts")
...
q2 <- read_rds("saved/11_transport2.rds")

kabble(q2, caption = "Extrapolation when two contexts differ on $W$ and $W$ is not observable in target context")
