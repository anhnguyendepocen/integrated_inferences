...
frank <- lapply(frank, function(d) select(d, - Case.estimand))
...
```{r frank4, echo = FALSE}
kabble(frank[[4]][1:4,-c(1,3)], caption = "Clue is informative after combining studies linking $K$ to $Z$ and $Z$ to $Y$", digits = 2)
```
...
if(do_diagnosis)
  ...
  write_rds("saved/ch11_copobs.rds")

model <-   read_rds("saved/ch11_copobs.rds")
...
read_rds("saved/11_fusionestimands.rds") %>% 
  ...
  kabble(caption = "Estimands in different sites", digits = 2)
...
kabble(summary(x)[[1]], digits = 3, caption = "Inferences on the ATE from differences in means")
```
...
if(do_diagnosis){
  write_rds(update_model(model, data), "saved/11_exp_obs.rds")
  }
```
...
kabble(caption = "Estimates on the ATE for observational ($R=0$) and experimental ($R=1$) set.")
```
...
result2 %>%
    select(-Case.estimand) %>%
kabble(caption = "Effects of $X$ conditional on $X$ for units that were randomly assigned or not.  Effects of $X$ do not depend on $X$ in the experimental group, but they do in the observational group becuase of seld selection. ")
```
...
hj_ggdag(model=model,nodesize = 20)
...
read_rds("saved/11_appev2.rds")  %>% 
  select(-Case.estimand) %>%
  kabble(caption = "Priors and true values (parameters) for three estimands: the frequency of $W$, the effect of $X$ on $Y$, and the effect conditional on $W=1$")
...
q <- read_rds("saved/11_transport.rds")

q %>%  select(-Case.estimand) %>%

kabble(caption = "Extrapolation when two sites differ on $W$ and $W$ is observable in both contexts")
...
if(do_diagnosis){
  ...
  write_rds(query_model(transport2,
            ...
            given = c("Context==0", "Context==1"),
            using = c("posteriors", "parameters"), expand_grid = TRUE),
            "saved/11_transport2.rds")
}

read_rds("saved/11_transport2.rds") %>%  select(-Case.estimand) %>%
kabble(caption = "Extrapolation when two contexts differ on $W$ and $W$ is not observable in target context")
