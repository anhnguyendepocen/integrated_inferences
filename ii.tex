...
\KeywordTok{library}\NormalTok{(CQtools)}
...
\NormalTok{my_design <-}
\StringTok{  }
\StringTok{  }\KeywordTok{causal_model_designer}\NormalTok{(}
...
\caption{\label{tab:data}Sample Data (shown in compact form)}
\centering
...
\NormalTok{model_priors <-}\StringTok{  }\NormalTok{model }\OperatorTok{%>%}
\StringTok{  }
\StringTok{  }\KeywordTok{set_priors}\NormalTok{(}\DataTypeTok{node =} \StringTok{"S"}\NormalTok{,}
             \DataTypeTok{statement =} \StringTok{"S[]==1"}\NormalTok{,}
             \DataTypeTok{alphas =} \DecValTok{10}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }
...
\caption{\label{tab:cinfer4}\label{possible_outcomespimd_i1} Table shows possible data patterns for P and M given I = 1 and D = 1 together with the probability of observing each data realization given data is sought on a variable and the posterior on the query (does $I$ have a positive effect on $D$) given that data realization.}
\centering
...
\hline
I1M0D1 & 1 &  & 0 & 1 & 0.000 & 0.029\\
\hline
I1M1D1 & 1 &  & 1 & 1 & 0.150 & 0.167\\
\hline
I1P0D1 & 1 & 0 &  & 1 & 0.231 & 0.054\\
\hline
I1P1D1 & 1 & 1 &  & 1 & 0.088 & 0.142\\
\hline
I1D1 & 1 &  &  & 1 & 0.128 & 0.196\\
\hline
\end{tabular}
\end{table}
...
\includegraphics{ii_files/figure-latex/graph-1.pdf} \includegraphics{ii_files/figure-latex/graph-2.pdf} \includegraphics{ii_files/figure-latex/graph-3.pdf} \includegraphics{ii_files/figure-latex/graph-4.pdf} \includegraphics{ii_files/figure-latex/graph-5.pdf} \includegraphics{ii_files/figure-latex/graph-6.pdf} \includegraphics{ii_files/figure-latex/graph-7.pdf} \includegraphics{ii_files/figure-latex/graph-8.pdf}
...
\includegraphics{ii_files/figure-latex/graph-9.pdf} \includegraphics{ii_files/figure-latex/graph-10.pdf} \includegraphics{ii_files/figure-latex/graph-11.pdf} \includegraphics{ii_files/figure-latex/graph-12.pdf} \includegraphics{ii_files/figure-latex/graph-13.pdf} \includegraphics{ii_files/figure-latex/graph-14.pdf} \includegraphics{ii_files/figure-latex/graph-15.pdf}
...
\caption{\label{tab:unnamed-chunk-19}Doubly decisive test}
\centering
\begin{tabular}[t]{l|l|l|r|r}
\hline
Query & Given & Using & mean & sd\\
...
\caption{\label{tab:unnamed-chunk-19}Hoop test}
\centering
\begin{tabular}[t]{l|l|l|r|r}
\hline
Query & Given & Using & mean & sd\\
...
\caption{\label{tab:unnamed-chunk-19}Smoking gun test}
\centering
\begin{tabular}[t]{l|l|l|r|r}
\hline
Query & Given & Using & mean & sd\\
...
\caption{\label{tab:unnamed-chunk-19}Straw in the wind test}
\centering
\begin{tabular}[t]{l|l|l|r|r}
\hline
Query & Given & Using & mean & sd\\
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{agnostic_data <-}\StringTok{ }\KeywordTok{make_model}\NormalTok{(}\StringTok{"X->M->Y"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{set_parameters}\NormalTok{(}\DataTypeTok{node =} \KeywordTok{c}\NormalTok{(}\StringTok{"M"}\NormalTok{, }\StringTok{"Y"}\NormalTok{), }\DataTypeTok{parameters =} \KeywordTok{list}\NormalTok{(}\KeywordTok{c}\NormalTok{(.}\DecValTok{1}\NormalTok{, }\DecValTok{0}\NormalTok{, }\FloatTok{.8}\NormalTok{, }\FloatTok{.1}\NormalTok{),}
                                                       \KeywordTok{c}\NormalTok{(.}\DecValTok{1}\NormalTok{, }\DecValTok{0}\NormalTok{, }\FloatTok{.8}\NormalTok{, }\FloatTok{.1}\NormalTok{))) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{simulate_data}\NormalTok{(}\DataTypeTok{n =} \DecValTok{1000}\NormalTok{)}

\ControlFlowTok{if}\NormalTok{(do_diagnosis)\{}
  
\NormalTok{agnostic_XMY <-}\StringTok{ }\KeywordTok{make_model}\NormalTok{(}\StringTok{"X->M->Y<-X; X<->M; X<->Y; M <->Y"}\NormalTok{)}\OperatorTok{%>%}
\StringTok{  }\KeywordTok{update_model}\NormalTok{(agnostic_data)}

\KeywordTok{write_rds}\NormalTok{(agnostic_XMY, }\StringTok{"ch14_agnostic.rds"}\NormalTok{)}


\NormalTok{agnostic_query <-}\StringTok{  }\KeywordTok{query_model}\NormalTok{(agnostic_XMY,}
              \DataTypeTok{query =} \StringTok{"Y[X=1] > Y[X=0]"}\NormalTok{, }
              \DataTypeTok{given =} \KeywordTok{c}\NormalTok{(}\OtherTok{TRUE}\NormalTok{, }\StringTok{"M==0"}\NormalTok{, }\StringTok{"M==1"}\NormalTok{),}
              \DataTypeTok{using =} \StringTok{"posteriors"}\NormalTok{)}

\KeywordTok{write_rds}\NormalTok{(agnostic_query, }\StringTok{"ch14_agnostic_query.rds"}\NormalTok{)}


\NormalTok{\}}

\NormalTok{agnostic_query <-}\StringTok{ }\KeywordTok{read_rds}\NormalTok{(}\StringTok{"ch14_agnostic_query.rds"}\NormalTok{)}

\NormalTok{agnostic_query }\OperatorTok{%>%}\StringTok{ }\KeywordTok{kable}\NormalTok{(}\DataTypeTok{caption =} \StringTok{"Conditional inferences from an update agnostic model given a true model in which $X$ causes $M$ and $M$ causes $Y$"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{table}

\caption{\label{tab:unnamed-chunk-20}Conditional inferences from an update agnostic model given a true model in which $X$ causes $M$ and $M$ causes $Y$}
\centering
\begin{tabular}[t]{l|l|l|r|r}
\hline
Query & Given & Using & mean & sd\\
\hline
Q 1 & - & posteriors & 0.398 & 0.092\\
\hline
Q 1 & M==0 & posteriors & 0.395 & 0.125\\
\hline
Q 1 & M==1 & posteriors & 0.400 & 0.130\\
\hline
\end{tabular}
\end{table}

This example illustrates the Cartwight idea we pointed to in Chapter 2: no causes in, no causes out \citet{cartwright1994nature}.
...
\hypertarget{justification-from-experimental-designs}{%
\section{Justification from experimental designs}\label{justification-from-experimental-designs}}
...
\includegraphics{ii_files/figure-latex/ch14Qornot-1.pdf}
...
\caption{\label{tab:unnamed-chunk-21}Regression coefficient on $X$ given $M=0$ and $M=1$}
\centering
...
\hypertarget{computational-clues}{%
\subsection{Computational clues}\label{computational-clues}}
...
\includegraphics{ii_files/figure-latex/unnamed-chunk-22-1.pdf} \includegraphics{ii_files/figure-latex/unnamed-chunk-22-2.pdf}
...
\caption{\label{tab:unnamed-chunk-24}Inferences on the probability that $X$ caused $Y$ upon seeing $M=0$ or $M=1$ for a range of possible values of $\lambda^Y_{10}$}
\centering
\begin{tabular}[t]{r|r|r|r}
\hline
