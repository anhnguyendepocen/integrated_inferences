...
Consider a situation in which our model is $X \rightarrow M \rightarrow Y$. Suppose, further, that we restrict the nodal types such that $X$ cannot have a negative effect on $M$, and $M$ cannot have a negative effect on $Y$. Imagine then that we begin by collecting only $X,Y$ data on six cases and obtain the following data pattern:
...
From the probability of each data type (given the model and the $X,Y$ data seen so far) and the posterior variance given each data realization, the implied *expected* variance is  easily calculated. These are summarized in Table \@ref(tab:exppostvar):
...
In this section, we illustrate the model-based approach to case-selection more extensively by applying it to a wide range of models, queries, and case-selection strategies. We generate the results displayed here using `CausalQueries` together with `CQTools`.
