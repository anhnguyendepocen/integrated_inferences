...
Along the $N=2$ vertical in each chain-model graph, we show the potential shifts in our beliefs from two stragies: examine two on-the-line cases, an $X=Y=0$ case and an $X=Y=1$ case; and examine two off-the line cases, one $X=1, Y=0$ case and one $X=0, Y=1$ case. We plot two points for each strategy (for each query), representing the inference we would draw from two possible realizations of $M$. While unlabelled, the higher dot represents the inference we would draw if $M$ varies positively with $X$; the lower dot represents the inference we would draw if $M=1$ in both cases.
...
The situation changes somewhat, however, if we come in with informative beliefs at the lower level. In the restricted-two-path set of graphs, we see results for a two-path model in which we have imposed stronger beliefs about the direction of causation at the lower level, ruling out negative $X \rightarrow M$ and negative $M \rightarrow Y$ effects. Now we see that we can learn from our $N=2$ strategies, though considerably more weakly than for the chain model. What seems to be happening here is that the restrictions make observations of $M$ quite a lot more informative about the operation of the indirect pathway. For instance, now, observing $M=0$ in an $X=1, Y=1$ case, for instance, is definitive evidence *against* causation along the indirect path in that case. So, now, our beliefs about causation along the indirect pathway can shift by a sufficient magnitude that the updating about the direct pathway cannot fully offset it. So evidence for (against) the operation of the indirect pathway registers, on net, as evidence for (against) the operation of a total $X \rightarrow Y$ effect.
