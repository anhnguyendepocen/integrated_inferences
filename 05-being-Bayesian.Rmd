...
First the strength of our prior level of confidence in the hypothesis, $\Pr(H)$. The greater the prior likelihood that our hypothesis is true, the greater the chance that new data consistent with the hypothesis has *in fact* been generated by a state of the world implied by the hypothesis.
...
###  Bayes' Rule for Continuous Parameters and The Dirichlet family 
...
For the framework developed in this book, we will often be interested in forming beliefs about the *share* of units that are of a particular type. For this type of question we will make quite heavy use of "Dirichlet" distributions: a family of distributions that capture beliefs about shares. 
...
```{r Betas, fig.height = 8, fig.width = 8, echo = FALSE, fig.cap="Beta distributions"}
  ...
  facet_wrap(alpha ~ beta, scales = "free_y", labeller = label_both) + ylab("density")
```
...
We thus shift our beliefs from a prior of $\frac13$ to a posterior of $\frac12$. In contrast had we *not* observed the clue our posterior would have been 0.^[We develop these ideas in more detail in Chapter \@ref(justifying).] 
...
write_rds(list(P1=P1, K1=K1, K2=K2), "saved/05_posteriors.rds")
}
L <- read_rds("saved/05_posteriors.rds")
