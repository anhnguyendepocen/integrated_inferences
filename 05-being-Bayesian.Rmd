...
Note, the denominator does not require a listing of all possible hypotheses, just an exhaustive collection of hypotheses. For example we might have the notion of the probability that the accused's fingerprints would be on the door if she were or were guilty without having to decompose the "not guilty" into a set of hypotheses regarding who else might be guilty. More subtly  *while the hypotheses that enter the formula do have to be mutually exclusive*, that does not prevent you from drawing inferences  about hypotheses that are not mutually exclusive. For instance you might for posteriors over which of 100 people is guilty, but then use the posterior to ask what is the probability that a man is guilty, or that an old person is guilty. The fact that "man" and "old" are not mutually exclusive in no way means you cannot try to learn about these hypotheses.      
